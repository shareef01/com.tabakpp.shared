    ...
    <div className={cn(
      ...
      <svg className="absolute inset-0 w-full h-full -rotate-90 overflow-visible p-1.5" viewBox="0 0 100 100">
        ...
        <motion.circle
          ...
          className={cn("transition-colors duration-1000", isL ? "text-danger" : "text-accent")}
          strokeLinecap="round"
        />
      </svg>
      <HeartPulse
        size={isSmall ? 16 : (isMedium ? 20 : 24)}
        className={cn("transition-all duration-1000", isL ? "text-danger scale-110" : "text-accent animate-pulse")}
      />
    </div>
  );
});
