...
const APP_VERSION = "18.0.0-SETTINGS-PRO";
          ...
          {activeTab === 'track' && (
            <motion.div key="track" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ type: 'spring', damping: 20, stiffness: 100 }} className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                 ...
                 <button onClick={() => setShowAdd(true)} className="bg-white/[0.01] rounded-[48px] border-2 border-dashed border-white/[0.05] flex flex-col items-center justify-center space-y-5 hover:bg-accent/5 hover:border-accent/20 transition-all min-h-[520px] group shadow-2xl">
...
// --- CORE UI COMPONENTS (iOS OPTIMIZED) ---
...
const TrackerCard = ({ config, count, onInc, onDec, index }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.05, type: 'spring', damping: 15 }} className={cn("bg-white/[0.02] rounded-[56px] border-2 p-10 flex flex-col items-center justify-between min-h-[520px] transition-all duration-700 group relative overflow-hidden shadow-2xl font-inter", isL ? "border-danger/30 shadow-[0_0_60px_rgba(248,113,113,0.1)]" : "border-white/[0.03] hover:border-accent/20")}>
      <span className="text-[10px] font-black text-white/20 tracking-[0.6em] uppercase relative z-10 font-inter">Target: {config.limit}</span>
      ...
      <div className="w-full flex justify-between items-center pt-8 mt-auto relative z-10 px-2">
        <button onClick={onDec} className="w-18 h-18 rounded-[24px] bg-white/[0.04] border border-white/[0.05] flex items-center justify-center text-white/30 hover:text-white active:scale-90 transition-all shadow-xl font-inter"><Minus size={32} strokeWidth={3} /></button>
        <button onClick={onInc} className={cn("w-18 h-18 rounded-[28px] flex items-center justify-center text-black active:scale-90 transition-all shadow-[0_20px_50px_var(--accent-rgb)] font-inter", isL ? "bg-danger shadow-[0_20px_50px_rgba(248,113,113,0.6)]" : "bg-accent")} style={{'--accent-rgb': 'rgba(0,210,255,0.4)'}}><Plus size={32} strokeWidth={4} /></button>
      </div>
    </motion.div>
  );
};
...
/**
 * <ProtocolListItem />
 * REFACTORED: Strict iPhone 15 Pro Max Row Layout.
 * FIX: 'flex-1' text stack pushes action buttons to far right, preventing overlap.
 * FIX: Removes bulky background boxes from rearrange arrows.
 */
const ProtocolListItem = ({ config, idx, total, onReo, onEdit, onDel }) => (
  <div className="flex flex-row justify-between items-center w-full p-6 bg-[#121316] rounded-[32px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl overflow-hidden min-h-[100px] gap-6">
    <div className="flex flex-row items-center gap-6 min-w-0 flex-1">
      {/* Component 1: Controls (Far Left) - Vertical stack, no boxes */}
      <div className="flex flex-col gap-2 shrink-0 items-center justify-center py-2">
        <button
          onClick={() => onReo(config.id, 'up')}
          disabled={idx === 0}
          className="text-white/20 hover:text-accent disabled:opacity-0 transition-all active:scale-75 p-1"
          aria-label="Move Up"
        >
      ...
      {/* Component 2: Main Icon (Center-Left) - Perfectly centered in squircle */}
      <div className="w-14 h-14 bg-white/[0.03] border border-white/5 rounded-[18px] flex items-center justify-center text-accent/60 group-hover:text-accent transition-all shrink-0 shadow-inner">
        {config.type.startsWith('JOINT') ? <Crown size={28} /> : (config.type === 'SIMPLE' ? <Activity size={28} /> : <Zap size={28} />)}
      </div>

      {/* Component 3: Text Stack (Center) - FLEX-1 pushes action buttons to the right */}
      <div className="flex flex-col gap-0.5 min-w-0 flex-1 text-left">
        <span className="text-xl font-[900] tracking-tight uppercase group-hover:text-white transition-colors truncate leading-tight font-inter">
          {config.name}
        </span>
        <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] leading-relaxed font-inter opacity-40">
          Target: {config.limit}
        </span>
      </div>
    </div>

    {/* Component 4: Action Buttons (Far Right) - Anchored to trailing edge */}
    <div className="flex flex-row items-center gap-4 shrink-0">
      <button
        onClick={() => onEdit(config)}
        className="w-11 h-11 rounded-[14px] bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/30 hover:text-accent hover:border-accent/20 transition-all shadow-md active:scale-90"
      ...
      <button
        onClick={onDel}
        className="w-11 h-11 rounded-[14px] bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/30 hover:text-danger hover:border-danger/20 transition-all shadow-md active:scale-90"
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/85 backdrop-blur-2xl">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className="bg-[#121318] border border-white/10 rounded-[56px] w-full max-w-lg p-10 flex flex-col shadow-2xl relative overflow-hidden font-inter" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 2rem)' }}>
          ...
          <button onClick={onAction} className="w-full h-18 md:h-20 bg-accent text-black font-[1000] uppercase tracking-[0.4em] rounded-[24px] shadow-2xl active:scale-95 transition-all flex items-center justify-center px-6 font-inter"><span className="whitespace-nowrap text-xs md:text-sm font-black">{actionLabel}</span></button>
        </motion.div>
      </div>
    )}
  </AnimatePresence>
    ...
    <IPhoneModifyModal isOpen={isOpen} onClose={onClose} title={title} actionLabel={initialData ? "Apply Modification" : "Authorize logic"} onAction={() => onApply({ name: n, limit: parseInt(l) || 20, type: t })}>
      <div className="space-y-10 text-left">
        <div className="relative group">
          <span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Descriptor</span>
        ...
        <div className="relative group">
          <span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Threshold</span>
        ...
        <div className="space-y-5">
          <span className="text-[10px] font-black uppercase tracking-[0.8em] text-white/20 ml-2 font-inter">Visual Schematic</span>
          <div className="grid grid-cols-2 gap-5">
            {['CIGARETTE', 'SIMPLE', 'JOINT_KING', 'JOINT_QUEEN'].map(x => (
              <button key={x} onClick={() => setT(x)} className={cn("h-18 rounded-[32px] border-2 font-black text-[12px] uppercase tracking-widest transition-all flex items-center justify-center gap-4 active:scale-95 font-inter", t === x ? "border-accent bg-accent/10 text-accent shadow-[0_0_30px_rgba(0,210,255,0.2)]" : "border-white/5 text-white/20 hover:border-white/10")}>{x.includes('KING') && <Crown size={18} />} {x.replace('_',' ')}</button>
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12 font-inter">
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[48px] shadow-[0_40px_100px_rgba(0,0,0,0.4)]">
          <div className="flex justify-between items-start mb-12">
             <div className="space-y-2 text-left">
                <h3 className="text-[10px] font-black text-white/30 tracking-[0.8em] uppercase font-inter">Visual Stream</h3>
                <span className="text-3xl font-[1000] tracking-tighter uppercase font-inter">Registry Logs</span>
             </div>
       ...
       <div className="space-y-8 pt-12">
          <h4 className="text-[10px] font-black text-white/20 tracking-[1em] uppercase px-4 text-left font-inter">Timeline Feed</h4>
          {logs.map((log, i) => (
...
const SettingsScreen = ({ configs, user, settings, onAdd, onReo, onEditP, onUpd, onDel }) => {
  ...
  const handleUpload = (e) => {
    ...
    reader.onload = async (ev) => {
       ...
       try {
          ...
          setLoading(false);
          window.location.reload(); // Refresh to sync avatar everywhere
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-12 max-w-3xl mx-auto font-inter">
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-12 shadow-2xl font-inter">
          <div className="flex flex-col items-center gap-10">
             <div className="relative group cursor-pointer" onClick={() => fileRef.current?.click()}>
                ...
                <div className="absolute -bottom-3 -right-3 w-14 h-14 bg-white text-black rounded-[22px] flex items-center justify-center shadow-2xl active:scale-90 transition-all border-4 border-[#020202]"><Camera size={24} strokeWidth={2.5} /></div>
                <input type="file" ref={fileRef} className="hidden" accept="image/*" onChange={handleUpload} />
             </div>
             <div className="w-full space-y-10">
                <div className="relative group text-left">
                  <span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Identity Label</span>
                  <input value={n} onChange={e=>setN(e.target.value)} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-lg font-[1000] focus:border-accent/40 outline-none transition-all font-inter" />
                </div>
                <button onClick={() => updateProfile(auth.currentUser, { displayName: n })} className="w-full h-20 bg-white text-black font-[1000] uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-4 font-inter">Commit Profile <ChevronRight size={24} /></button>
             </div>
          </div>
       </Card>

       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-10 shadow-2xl font-inter">
          <div className="space-y-10">
             <div className="px-2 text-left">
                <h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/30 mb-2 font-inter">Interface Schematics</h3>
                <span className="text-3xl font-[1000] tracking-tighter uppercase font-inter">Accent Spectrum</span>
             </div>
             ...
             <button onClick={applyTheme} className="w-full h-20 bg-accent text-black font-[1000] uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all flex items-center justify-center gap-4 shadow-2xl font-inter"><Zap size={22} fill="currentColor" /> Apply Scheme</button>
          </div>
       </Card>
       ...
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-10 shadow-2xl font-inter">
          <div className="flex items-center justify-between px-2">
             <div className="space-y-2 text-left">
                <h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/30 font-inter">Active Schematics</h3>
                <span className="text-3xl font-[1000] tracking-tighter uppercase font-inter">Protocols</span>
             </div>
       ...
       <button onClick={() => signOut(auth)} className="w-full h-22 border-2 border-danger/30 text-danger font-[1000] uppercase tracking-[0.6em] rounded-[32px] hover:bg-danger/5 transition-all flex items-center justify-center gap-6 active:scale-95 shadow-2xl shadow-danger/5 font-inter font-black"><LogOut size={28} /> Terminate Sessions</button>
    </motion.div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-black/80 backdrop-blur-xl font-inter">
       <div className="bg-[#121318] border border-white/10 rounded-[64px] w-full max-w-[560px] p-16 space-y-12 shadow-2xl font-inter">
          ...
          <div className="space-y-10 max-h-[450px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-accent/30 font-inter">
             {configs.map(x => (
                <div key={x.id} className="relative group text-left">
      ...
      <div className="w-full max-w-[450px] space-y-16 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/[0.02] border border-white/[0.05] p-12 rounded-[56px] space-y-10 shadow-2xl backdrop-blur-xl">
          <div className="space-y-8">
            ...
            {!isL && (
              <div className="relative group text-left font-inter">
                <span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Identity</span>
                <input value={n} onChange={e=>setN(e.target.value)} className="w-full h-18 bg-white/[0.03] border border-white/5 rounded-[24px] px-6 pt-5 text-sm font-bold focus:border-accent/40 outline-none transition-all font-inter" placeholder="Registry Name" />
              </div>
            )}
            <div className="relative group text-left font-inter">
              <span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Link</span>
              <input value={e} onChange={e=>setE(e.target.value)} className="w-full h-18 bg-white/[0.03] border border-white/5 rounded-[24px] px-6 pt-5 text-sm font-bold focus:border-accent/40 outline-none transition-all font-inter" placeholder="Email Address" />
            </div>
            <div className="relative group text-left font-inter">
              <span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Passkey</span>
              <input type="password" value={p} onChange={e=>setP(e.target.value)} className="w-full h-18 bg-white/[0.03] border border-white/5 rounded-[24px] px-6 pt-5 text-sm font-bold focus:border-accent/40 outline-none transition-all font-inter" placeholder="••••••••" />
            </div>
...
const ErrorView = ({ msg }) => <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter"><AlertCircle className="text-danger mb-12" size={120} strokeWidth={2} /><h2 className="text-5xl font-[1000] uppercase tracking-tighter leading-none mb-8 font-inter">Sync Failure</h2><p className="text-white/20 text-sm max-w-sm font-bold opacity-60 leading-relaxed uppercase tracking-widest mb-16 font-inter">{msg}</p><button onClick={() => window.location.reload()} className="rounded-[32px] font-[1000] uppercase tracking-[0.6em] px-20 h-24 bg-white text-black shadow-2xl active:scale-95 transition-all font-inter">Re-Link System</button></div>;
