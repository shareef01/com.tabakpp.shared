    ...
    >
      ...
      <main className="flex-1 overflow-y-auto pt-40 pb-44 px-6 max-w-6xl mx-auto w-full transition-all duration-500">
        <AnimatePresence mode="wait">
          {activeTab === 'tracker' && <TrackerScreen key="t" m={metrics} c={configs} onInc={onInc} onDec={onDec} isDark={settings.isDark} view={settings.layout} onAdd={() => setShowAdd(true)} />}
...
const CounterCard = ({ config, count, onInc, onDec, isC, isDark }) => {
  ...
  return (
    <Card className={cn("relative flex flex-col group transition-all duration-1000 p-12 overflow-hidden shadow-2xl", isL ? "bg-danger/[0.04] border-danger/50 shadow-[0_0_60px_rgba(248,113,113,0.15)]" : (isDark ? "bg-white/[0.03] border-white/5 shadow-black/20" : "bg-white border-black/5 shadow-black/5 shadow-2xl"), isC ? "min-h-[420px]" : "min-h-[580px]")}>
       ...
       <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-10">
          {(isCig || isJoint) ? (
             <div className={cn("relative w-full h-14 rounded-full overflow-hidden transition-all duration-1000 border shadow-2xl", isL ? "bg-danger border-danger/40" : (isDark ? "bg-[#111] border-white/5 shadow-[inset_0_4px_20px_rgba(0,0,0,0.8)]" : "bg-[#EEE] border-black/5 shadow-inner"))}>
                ...
                {!isL && <div className={cn("absolute right-[28%] inset-y-0 transition-all duration-700 shadow-xl", isCig ? "bg-white shadow-xl" : "bg-[#C8E6C9] shadow-xl")} style={{ left: `${p * 72}%` }} />}
       ...
       <div className="flex justify-center items-center space-x-12 relative z-20 pt-8"><motion.button whileTap={{ scale: 0.7 }} onClick={() => onDec(config.id)} className={cn("w-20 h-20 rounded-full flex items-center justify-center transition-all shadow-xl", isDark ? "bg-white/5 text-text-dim hover:text-white" : "bg-[#F5F5F7] text-black/40 hover:text-black border border-black/5 shadow-inner shadow-black/5")}><Minus size={32} /></motion.button><motion.button whileTap={{ scale: 0.9 }} onClick={() => onInc(config.id)} className={cn("w-20 h-20 rounded-full border-2 flex items-center justify-center transition-all shadow-2xl", isL ? "border-danger text-danger bg-danger/10 shadow-danger/20" : "border-accent/40 text-accent bg-accent/5 hover:border-accent hover:rotate-1")}><Plus size={36} /></motion.button></div>
    </Card>
  );
};
...
const SettingsScreen = ({ c, u, s, onAdd, onUpd, onReo, onDel }) => {
  ...
  return (
    <div className="flex flex-col space-y-12 pb-40 max-w-4xl mx-auto">
       <Card className={cn("p-14 relative overflow-hidden shadow-2xl", s.isDark ? "bg-white/[0.03] border-white/5 shadow-black" : "bg-white border-black/5 shadow-black/5 shadow-2xl")}><div className="flex items-center space-x-12 mb-16 relative z-10"><div className="w-36 h-36 bg-accent rounded-[56px] border-2 border-accent/20 flex items-center justify-center text-7xl font-[1000] text-bg-base shadow-2xl overflow-hidden shadow-accent/20 transition-all active:scale-95">{u.photoURL ? <img src={u.photoURL} className="w-full h-full object-cover" alt="p" /> : al.charAt(0)}</div><div className="space-y-4"><h4 className={cn("text-5xl font-[1000] tracking-tighter uppercase leading-none transition-colors", !s.isDark && "text-[#1D1D1F]")}>{al}</h4><div className="flex items-center space-x-4"><div className="h-2.5 w-2.5 rounded-full bg-accent animate-pulse shadow-[0_0_15px_var(--accent)]" /><span className="text-[12px] font-black uppercase tracking-[0.6em] text-accent font-bold font-inter">Vault Commander</span></div></div></div><div className="space-y-12 relative z-10"><Input label="Identifier" value={al} onChange={setAl} isDark={s.isDark} /><Input label="Objective" value={gl} onChange={setGl} isDark={s.isDark} /><Button className="w-full h-22 rounded-[36px] shadow-2xl text-[11px] font-[1000] uppercase active:scale-95 transition-all" onClick={() => onUpd({ displayName: al, goal: gl })}>Update Parameters</Button></div></Card>
...
const InsightCard = ({ Icon, label, val, suffix, color, index, isDark }) => (
  <StaggeredItem index={index}>
    <Card className={cn("flex flex-col items-center text-center py-16 transition-all duration-1000 shadow-2xl group shadow-black/5 shadow-black/10 shadow-2xl", isDark ? "bg-white/[0.03] border-white/5 shadow-black" : "bg-white border-black/5")}>
