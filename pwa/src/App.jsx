...
// --- GLOBAL CONSTANTS ---
const APP_VERSION = "18.6.0-BOOT-FIX";
...
// --- SUB-COMPONENTS (DEFINED BEFORE APP TO PREVENT REFERENCE ERRORS) ---

const TopBanner = ({ user, onProfileClick }) => (
  <header className="sticky top-0 z-[100] w-full backdrop-blur-md bg-black/70 border-b border-white/[0.03] transition-all" style={{ paddingTop: 'max(env(safe-area-inset-top), 1rem)', paddingBottom: '1.25rem' }}>
...
const TrackerCard = ({ config, count, onInc, onDec, index }) => {
  const isL = count >= config.limit; const isKing = config.type === 'JOINT_KING'; const isQueen = config.type === 'JOINT_QUEEN';
  return (
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center space-y-12 relative z-10 py-10">
        ...
        <div className="flex flex-col items-center">
          <div className="flex items-center gap-3">
            ...
            <motion.span key={count} initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className={cn("text-7xl font-[1000] tracking-tighter tabular-nums transition-all duration-700 leading-none font-inter", isL ? "text-danger drop-shadow-[0_0_30px_rgba(248,113,113,0.4)]" : "text-white")}>{count}</motion.span>
          </div>
...
const ProtocolListItem = ({ config, idx, total, onReo, onEdit, onDel }) => (
  <div className="flex flex-row justify-between items-center w-full p-6 md:p-8 bg-[#121316] rounded-[32px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl overflow-hidden min-h-[100px] gap-6">
    ...
    <div className="flex flex-row items-center gap-4 shrink-0">
      <button onClick={() => onEdit(config)} className="w-11 h-11 rounded-[14px] bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/30 hover:text-accent active:scale-90 transition-all"><Edit2 size={18} /></button>
      <button onClick={onDel} className="w-11 h-11 rounded-[14px] bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/30 hover:text-danger active:scale-90 transition-all"><Trash2 size={18} /></button>
    </div>
  </div>
);
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/85 backdrop-blur-2xl">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className="bg-[#121318] border border-white/10 rounded-[56px] w-full max-w-lg p-10 flex flex-col shadow-2xl relative overflow-hidden font-inter" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 2rem)' }}>
          ...
          <button onClick={onAction} className="w-full h-18 md:h-20 bg-accent text-black font-[1000] uppercase tracking-[0.4em] rounded-[24px] shadow-2xl active:scale-95 transition-all flex items-center justify-center px-6 font-black"><span className="whitespace-nowrap text-xs md:text-sm">{actionLabel}</span></button>
        </motion.div>
      </div>
    )}
  </AnimatePresence>
  ...
  return (
    <IPhoneModifyModal isOpen={isOpen} onClose={onClose} title={title} actionLabel={initialData ? "Apply Modification" : "Authorize logic"} onAction={() => onApply({ name: n, limit: parseInt(l) || 20, type: t })}>
      <div className="space-y-10 text-left">
        <div className="relative group"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40">Descriptor</span><input value={n} onChange={e=>setN(e.target.value)} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-lg font-[1000] focus:border-accent/40 outline-none transition-all" placeholder="Protocol Label" /></div>
        <div className="relative group"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40">Threshold</span><input type="number" value={l} onChange={e=>setL(e.target.value)} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-lg font-[1000] focus:border-accent/40 outline-none transition-all" /></div>
        <div className="space-y-5"><span className="text-[10px] font-black uppercase tracking-[0.8em] text-white/20 ml-2">Visual Schematic</span><div className="grid grid-cols-2 gap-5">{['CIGARETTE', 'SIMPLE', 'JOINT_KING', 'JOINT_QUEEN'].map(x => ( <button key={x} onClick={() => setT(x)} className={cn("h-18 rounded-[32px] border-2 font-black text-[12px] uppercase tracking-widest transition-all flex items-center justify-center gap-4 active:scale-95", t === x ? "border-accent bg-accent/10 text-accent shadow-[0_0_30px_rgba(0,210,255,0.2)]" : "border-white/5 text-white/20 hover:border-white/10")}>{x.includes('KING') && <Crown size={18} />} {x.replace('_',' ')}</button> ))}</div></div>
      </div>
    </IPhoneModifyModal>
  );
};
...
const HistoryScreen = ({ logs, configs, m, onEdit, userId, today }) => {
  const onDelete = async (logDate) => { if (window.confirm("Delete record?")) await deleteDoc(doc(db, 'users', userId, 'logs', logDate)); };
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 font-inter">
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[48px] shadow-2xl">
          <div className="flex justify-between items-start mb-12 text-left"><div className="space-y-2 text-left"><h3 className="text-[10px] font-black text-white/30 tracking-[0.8em] uppercase">Visual Stream</h3><span className="text-3xl font-[1000] tracking-tighter uppercase">Registry Logs</span></div><div className="p-4 bg-accent/10 rounded-[20px] text-accent"><BarChart3 size={32} strokeWidth={2.5} /></div></div>
          <div className="h-72 w-full"><ResponsiveContainer width="100%" height="100%"><LineChart data={logs.slice(0, 7).reverse().map(l => ({ name: new Date(l.logDate).toLocaleDateString(undefined, {weekday:'short'}).toUpperCase(), val: Object.values(l.counts || {}).reduce((a,b)=>a+b, 0) }))}><CartesianGrid strokeDasharray="8 8" stroke="#ffffff03" vertical={false} /><XAxis dataKey="name" stroke="#6b7280" fontSize={10} axisLine={false} tickLine={false} tick={{fontWeight:900}} dy={15} /><Tooltip contentStyle={{ background: '#121316', border: 'none', borderRadius: '24px', fontSize: '12px' }} /><Line type="monotone" dataKey="val" stroke="var(--accent)" strokeWidth={8} dot={{ r: 8, fill: 'var(--accent)', strokeWidth: 5, stroke: '#0a0a0c' }} /></LineChart></ResponsiveContainer></div>
       </Card>
...
const SettingsScreen = ({ configs, user, settings, onAdd, onReo, onEditP, onUpd, onDel }) => {
  ...
  const handleUpload = (e) => {
    ...
    reader.onload = async (ev) => {
       ...
       try { await updateProfile(auth.currentUser, { photoURL: url }); await setDoc(doc(db, 'users', user.uid), { photoURL: url }, { merge: true }); setLoading(false); window.location.reload(); }
    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 max-w-3xl mx-auto font-inter text-left">
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-12 shadow-2xl">
          <div className="flex flex-col items-center gap-10">
             <div className="relative group cursor-pointer" onClick={() => fileRef.current?.click()}><div className={cn("w-40 h-40 rounded-[48px] bg-accent/5 border-2 border-accent/20 flex items-center justify-center overflow-hidden shadow-2xl transition-all duration-700", loading && "animate-pulse opacity-50")}>{user.photoURL ? <img src={user.photoURL} className="w-full h-full object-cover" alt="p" /> : <User size={64} className="text-accent" />}</div><div className="absolute -bottom-3 -right-3 w-14 h-14 bg-white text-black rounded-[22px] flex items-center justify-center shadow-2xl border-4 border-[#020202]"><Camera size={24} strokeWidth={2.5} /></div><input type="file" ref={fileRef} className="hidden" accept="image/*" onChange={handleUpload} /></div>
             <div className="w-full space-y-10"><div className="relative group"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40">Identity Label</span><input value={n} onChange={e=>setN(e.target.value)} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-lg font-[1000] focus:border-accent/40 outline-none transition-all font-inter" /></div><button onClick={() => updateProfile(auth.currentUser, { displayName: n })} className="w-full h-20 bg-white text-black font-black uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-4">Commit Profile <ChevronRight size={24} /></button></div>
          </div>
       </Card>
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-10 shadow-2xl">
          <div className="space-y-10"><div className="px-2"><h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/30 mb-2">Interface Schematics</h3><span className="text-3xl font-[1000] tracking-tighter uppercase">Accent Spectrum</span></div><div className="grid grid-cols-3 gap-6">{ACCENTS.map(x => ( <button key={x.v} onClick={() => setLa(x.v)} className={cn("h-16 rounded-[24px] border-2 transition-all duration-500 relative flex items-center justify-center", la === x.v ? "border-white scale-105 shadow-2xl" : "border-white/[0.05] opacity-40 hover:opacity-100")} style={{ backgroundColor: x.v }}>{la === x.v && <Check size={24} className="text-white drop-shadow-md" strokeWidth={4} />}</button> ))}</div><button onClick={applyTheme} className="w-full h-20 bg-accent text-black font-[1000] uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all flex items-center justify-center gap-4 shadow-2xl"><Zap size={22} fill="currentColor" /> Apply Scheme</button></div>
       </Card>
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-10 shadow-2xl">
          <div className="flex items-center justify-between px-2"><div className="space-y-2 text-left font-inter"><h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/30">Active Schematics</h3><span className="text-3xl font-[1000] tracking-tighter uppercase font-inter">Protocols</span></div><button onClick={onAdd} className="p-5 bg-accent text-black rounded-[24px] hover:brightness-110 active:scale-90 transition-all shadow-2xl"><Plus size={32} strokeWidth={3} /></button></div>
          <div className="space-y-6">{configs.sort((a,b)=>a.order-b.order).map((c, idx) => ( <ProtocolListItem key={c.id} config={c} idx={idx} total={configs.length} onReo={onReo} onEdit={onEditP} onDel={() => onDel(c.id)} /> ))}</div>
       </Card>
...
const ProfileModal = ({ user, onClose }) => (
  <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-black/80 backdrop-blur-xl font-inter">
    <motion.div initial={{ opacity: 0, scale: 0.9, y: 50 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 50 }} className="bg-[#121318] border border-white/10 rounded-[64px] w-full max-w-[480px] p-16 relative z-10 shadow-2xl text-center space-y-10">
      ...
      <div><h4 className="text-4xl font-[1000] uppercase tracking-tighter leading-none font-inter">{user.displayName || 'Registry User'}</h4><p className="text-white/20 text-sm font-black tracking-[0.4em] uppercase mt-4 font-inter">{user.email}</p></div>
...
const EditOverlay = ({ log, configs, onClose, user }) => {
  ...
  const handle = async () => { await setDoc(doc(db, 'users', user.uid, 'logs', log.logDate), { counts: c, logDate: log.logDate }, { merge: true }); onClose(); };
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-black/80 backdrop-blur-xl font-inter">
       <div className="bg-[#121318] border border-white/10 rounded-[64px] w-full max-w-[560px] p-16 space-y-12 shadow-2xl font-inter">
          <div className="flex justify-between items-center"><h3 className="text-3xl font-[1000] uppercase tracking-tighter">Override Daily Log</h3><button onClick={onClose} className="p-4 bg-white/5 rounded-full"><X size={32} /></button></div>
          ...
          <div className="space-y-10 max-h-[450px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-accent/30 font-inter">{configs.map(x => ( <div key={x.id} className="relative group text-left font-inter"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest font-inter">{x.name} Units</span><input type="number" value={c[x.id] || 0} onChange={e=>setC({...c, [x.id]: parseInt(e.target.value) || 0})} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-2xl font-[1000] focus:border-accent/40 outline-none transition-all font-inter" /></div> ))}</div>
          <button onClick={handle} className="w-full h-22 bg-accent text-black font-[1000] uppercase tracking-[0.6em] rounded-[32px] shadow-2xl active:scale-95 transition-all font-inter font-black">Commit Override</button>
       </div>
    </div>
  );
};
  ...
  return (
    <div className="min-h-screen bg-[#020202] flex items-center justify-center p-8 text-white font-inter relative overflow-hidden font-inter">
      ...
      <div className="w-full max-w-[450px] space-y-16 relative z-10">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', damping: 20 }} className="flex flex-col items-center text-center"><div className="w-24 h-24 bg-accent rounded-[36px] flex items-center justify-center mb-10 shadow-[0_0_80px_rgba(0,210,255,0.4)] text-black"><Zap size={48} fill="currentColor" /></div><h1 className="text-7xl font-[1000] tracking-tighter uppercase leading-none drop-shadow-2xl">TABAK<span className="text-accent">++</span></h1></motion.div>
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white/[0.02] border border-white/[0.05] p-12 rounded-[56px] space-y-10 shadow-2xl backdrop-blur-xl">
          <div className="space-y-8">{err && <div className="p-6 bg-danger/10 border border-danger/20 rounded-[24px] text-danger text-[11px] font-[1000] text-center uppercase tracking-widest">{err}</div>}{!isL && ( <div className="relative group text-left"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40">Identity</span><input value={n} onChange={e=>setN(e.target.value)} className="w-full h-18 bg-white/[0.03] border border-white/5 rounded-[24px] px-6 pt-5 text-sm font-bold focus:border-accent/40 outline-none transition-all font-inter" placeholder="Registry Name" /></div> )}<div className="relative group text-left"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40">Link</span><input value={e} onChange={e=>setE(e.target.value)} className="w-full h-18 bg-white/[0.03] border border-white/5 rounded-[24px] px-6 pt-5 text-sm font-bold focus:border-accent/40 outline-none transition-all font-inter" placeholder="Email Address" /></div><div className="relative group text-left"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40">Passkey</span><input type="password" value={p} onChange={e=>setP(e.target.value)} className="w-full h-18 bg-white/[0.03] border border-white/5 rounded-[24px] px-6 pt-5 text-sm font-bold focus:border-accent/40 outline-none transition-all font-inter" placeholder="••••••••" /></div><button className="w-full h-20 bg-accent text-black font-black uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-[0_20px_60px_rgba(0,210,255,0.4)] flex items-center justify-center font-inter" onClick={handle} disabled={loading}>{loading ? <Loader2 className="animate-spin" /> : (isL ? 'Sync Registry' : 'Join Registry')}</button></div>
          <button onClick={() => setIsL(!isL)} className="w-full text-[11px] font-[1000] text-white/20 uppercase tracking-[0.4em] hover:text-accent transition-all text-center font-inter">{isL ? "Request Access Entry" : "Return to Log In"}</button>
        </motion.div>
      </div>
    </div>
  );
...
const ErrorView = ({ msg }) => <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter"><AlertCircle className="text-danger mb-12" size={120} strokeWidth={2} /><h2 className="text-5xl font-[1000] uppercase tracking-tighter leading-none mb-8">Sync Failure</h2><p className="text-white/20 text-sm max-w-sm font-bold opacity-60 leading-relaxed uppercase tracking-widest mb-16">{msg}</p><button onClick={() => window.location.reload()} className="rounded-[32px] font-[1000] uppercase tracking-[0.6em] px-20 h-24 bg-white text-black shadow-2xl active:scale-95 transition-all font-inter font-black">Re-Link System</button></div>;
const LoadingView = () => <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center space-y-12 text-accent font-inter font-black"><Loader2 className="animate-spin" size={100} strokeWidth={3} /><span className="text-sm font-black tracking-[1.5em] uppercase text-accent animate-pulse ml-[1.2em]">Synchronizing Registry</span></div>;
