    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter">
          ...
          <h2 className="text-3xl font-[950] uppercase tracking-tighter">System Malfunction</h2>
          <p className="text-text-dim text-sm mt-4 mb-10 max-w-xs leading-relaxed font-bold opacity-60">{this.state.error?.message || "An unexpected UI error occurred."}</p>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    }, (err) => setAppError("Vault Access Denied: " + err.message));
  }, []);
  ...
  return (
    ...
    <div className={cn("flex flex-col min-h-screen bg-bg-base text-white font-inter selection:bg-accent/30 transition-all duration-700", !settings.isDark && "invert-[0.9] hue-rotate-180")} style={{ '--accent': settings.accent, '--accent-rgb': hexToRgb(settings.accent), fontSize: `${settings.fontScale}rem` }}>
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:bottom-10 md:left-1/2 md:-translate-x-1/2 md:w-[600px] pb-[env(safe-area-inset-bottom)] md:pb-0 z-40 px-4">
        <div className="bg-black/80 backdrop-blur-3xl border border-white/5 rounded-t-[40px] md:rounded-[40px] flex justify-around items-center h-24 md:h-22 px-6 shadow-[0_30px_60px_rgba(0,0,0,0.8)] border-white/5">
...
const CounterCard = ({ config, count, onInc, onDec, isC }) => {
  ...
  // Correct Burn Logic: The white body exists on the RIGHT and shrinks towards the filter on the RIGHT.
  // This means the LEFT edge of the white body moves right.
  ...
  return (
    <Card className={cn("relative flex flex-col group transition-all duration-1000 p-12 overflow-hidden", isL ? "bg-danger/[0.04] border-danger/50 shadow-[0_0_60px_rgba(248,113,113,0.15)]" : "hover:border-accent/30 border-white/5 shadow-2xl", isC ? "min-h-[380px]" : "min-h-[550px]")}>
       ...
       <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-12">
          ...
          ) : <div className={cn("text-white font-[950] tracking-tighter leading-none mb-10 transition-all", isC ? "text-8xl" : "text-[12rem]", isL ? "text-danger drop-shadow-[0_0_40px_rgba(248,113,113,0.4)]" : "text-white")}>{count}</div>}
