...
const App = () => {
  ...
  useEffect(() => {
    ...
    } catch (err) { setAppError("Sync Malfunction: " + err.message); }
  }, [user]);
  ...
  if (!user) return <AuthScreen isDark={settings.isDark} accent={settings.accent} />;
    ...
    >
            ...
            {showProfileMenu && (
              <>
                ...
                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className={cn("absolute right-0 mt-4 w-56 rounded-[32px] p-2 shadow-2xl border backdrop-blur-3xl z-[100]", settings.isDark ? "bg-black/90 border-white/10" : "bg-white/95 border-black/5 shadow-black/10")}>
                  ...
                  <button onClick={() => { signOut(auth); setShowProfileMenu(false); }} className="w-full flex items-center space-x-4 p-4 rounded-2xl hover:bg-danger/10 text-danger transition-colors text-sm font-[1000] uppercase tracking-widest border-t border-white/5 mt-1"><LogOut size={18} /><span>Terminal</span></button>
                </motion.div>
              </>
            )}
          </AnimatePresence>
      ...
      <main className="flex-1 overflow-y-auto pt-40 pb-44 px-6 max-w-6xl mx-auto w-full transition-all duration-500">
        <AnimatePresence mode="wait">
          {activeTab === 'tracker' && <TrackerScreen key="t" m={metrics} c={configs} onInc={onInc} onDec={onDec} view={settings.layout} isDark={settings.isDark} />}
      ...
      <nav className="fixed bottom-0 left-0 right-0 md:bottom-10 md:left-1/2 md:-translate-x-1/2 md:w-[600px] pb-[env(safe-area-inset-bottom)] md:pb-0 z-[110] px-4">
        <div className={cn("backdrop-blur-3xl border rounded-t-[40px] md:rounded-[40px] flex justify-around items-center h-24 md:h-22 px-6 shadow-2xl transition-all duration-500", settings.isDark ? "bg-black/80 border-white/5" : "bg-white/80 border-black/5 shadow-black/5 shadow-2xl")}>
      ...
      <AnimatePresence>
        ...
        {editTarget && <Overlay onClose={() => setEditTarget(null)} title="Correct Data" isDark={settings.isDark}><EditForm log={editTarget} configs={configs} onSave={async (d, c) => { await setDoc(doc(db, 'users', user.uid, 'logs', d), { counts: c }, { merge: true }); setEditTarget(null); }} isDark={settings.isDark} /></Overlay>}
      </AnimatePresence>
    </div>
    </ErrorBoundary>
  );
    ...
    <div className="min-h-screen bg-[#020202] flex items-center justify-center p-6 text-white font-inter">
      <div className="w-full max-w-md space-y-12">
        ...
        <Card className="space-y-6 bg-white/[0.03] border-white/5 p-10 backdrop-blur-lg shadow-2xl"><div className="flex flex-col space-y-6">
          ...
          {!isL && <Input label="Commander" value={n} onChange={setN} placeholder="Alias" isDark={true} />}
          <Input label="Vault ID" value={e} onChange={setE} placeholder="email@vault.com" isDark={true} />
          <Input label="Security" type="password" value={p} onChange={setP} placeholder="••••••••" isDark={true} />
          <Button className="w-full h-18 text-xs shadow-2xl font-black uppercase" onClick={handle} disabled={loading} style={{'--accent': accent}}>{loading ? <Loader2 className="animate-spin" /> : (isL ? 'Access Vault' : 'Init Vault')}</Button>
          <div className="flex flex-col space-y-4 items-center pt-2"><button onClick={() => setIsL(!isL)} className="text-[10px] font-black text-text-dim uppercase tracking-widest hover:text-accent transition-all">{isL ? "Request Access" : "Return to Point"}</button><button onClick={() => signInAnonymously(auth)} className="text-[10px] font-black text-accent/60 uppercase tracking-widest hover:text-accent transition-all">Observer Mode</button></div>
        </div></Card>
      </div>
    </div>
  );
...
const TrackerScreen = ({ m, c, view, isDark }) => (
  <div className="flex flex-col space-y-12 pb-10 max-w-5xl mx-auto">
    ...
    <div className={cn("grid gap-10", view === 'COMPACT' ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 lg:grid-cols-2")}>{c.sort((a,b) => a.order - b.order).map((config, i) => <StaggeredItem key={config.id} index={i+1}><CounterCard config={config} count={(m.todayLog.counts || {})[config.id] || 0} isC={view === 'COMPACT'} isDark={isDark} onInc={async () => { const ref = doc(db, 'users', auth.currentUser.uid, 'logs', new Date().toISOString().split('T')[0]); await setDoc(ref, { counts: { ...(m.todayLog.counts || {}), [config.id]: ((m.todayLog.counts || {})[config.id] || 0) + 1 } }, { merge: true }); }} onDec={async () => { const counts = m.todayLog.counts || {}; if (!counts[config.id] || counts[config.id] <= 0) return; const ref = doc(db, 'users', auth.currentUser.uid, 'logs', new Date().toISOString().split('T')[0]); await setDoc(ref, { counts: { ...counts, [config.id]: counts[config.id] - 1 } }, { merge: true }); }} /></StaggeredItem>)}</div>
  </div>
);
       ...
       <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-10">
          {(isCig || isJoint) ? (
             <div className={cn("relative w-full h-14 rounded-full overflow-hidden transition-all duration-1000 border shadow-2xl", isL ? "bg-danger border-danger/40" : (isDark ? "bg-[#111] border-white/5 shadow-[inset_0_4px_20px_rgba(0,0,0,0.8)]" : "bg-[#EEE] border-black/5 shadow-inner"))}>
                ...
                {!isL && count > 0 && <div className="absolute inset-y-0 w-3 bg-[#FF3D00] shadow-[0_0_20px_#FF3D00] z-20" style={{ left: `calc(${p * 72}% - 2px)` }} />}
    ...
    <div className="flex flex-col space-y-12 pb-40 max-w-4xl mx-auto">
       ...
       <Card className={cn("p-14 space-y-16 shadow-2xl", s.isDark ? "bg-white/[0.03] border-white/5 shadow-black" : "bg-white border-black/5 shadow-black/5")}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12"><Toggle icon={Moon} label="Obsidian Mode" active={s.isDark} onClick={() => onUpd({ isDark: !s.isDark })} isDark={s.isDark} /><Toggle icon={Clock} label="Night Owl Mode" active={s.nightOwl} onClick={() => onUpd({ nightOwl: !s.nightOwl })} isDark={s.isDark} /><Toggle icon={Grid} label="Neural Matrix" active={s.layout === 'COMPACT'} onClick={() => onUpd({ layout: s.layout === 'LARGE' ? 'COMPACT' : 'LARGE' })} isDark={s.isDark} />
             <div className={cn("p-10 rounded-[48px] border space-y-10 shadow-inner transition-all", s.isDark ? "bg-black/40 border-white/5" : "bg-[#F0F2F5] border-black/5 shadow-inner")}><div className="flex items-center justify-between px-2 text-xs font-[1000] uppercase tracking-[0.5em] opacity-60"><span>Accent Spectrum</span><Activity size={18} className="opacity-30" /></div><div className="flex flex-wrap gap-5">{ACCENTS.map(x => (
...
const Toggle = ({ icon: Icon, label, active, onClick, isDark }) => (
  <div onClick={onClick} className={cn("flex items-center justify-between p-10 rounded-[48px] border shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98] group cursor-pointer shadow-black/5", isDark ? "bg-white/[0.02] border-white/5" : "bg-white border-black/5")}>
     <div className="flex items-center space-x-7"><div className={cn("p-6 rounded-[32px] transition-all duration-700 shadow-2xl", isDark ? "bg-white/5 border border-white/5 shadow-black" : "bg-[#F0F2F5] border-black/5 shadow-inner")}><Icon size={32} className={cn(active ? "text-accent" : (isDark ? "text-white/40" : "text-black/40"))} /></div><span className={cn("text-sm font-[1000] uppercase tracking-[0.5em] leading-none transition-all font-black", !isDark && "text-[#1D1C1E]")}>{label}</span></div>
...
const InsightCard = ({ Icon, label, val, suffix, color, index, isDark }) => (
  <StaggeredItem index={index}>
    <Card className={cn("flex flex-col items-center text-center py-16 transition-all duration-1000 shadow-2xl group", isDark ? "bg-white/[0.03] border-white/5 shadow-black" : "bg-white border-black/5 shadow-black/5")}>
...
const ErrorView = ({ msg }) => <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter"><AlertCircle className="text-danger mb-6" size={48} /><h2 className="text-2xl font-black uppercase tracking-tighter leading-none mb-2 text-white font-black">System Terminal Fail</h2><p className="text-text-dim text-sm max-w-xs">{msg}</p><Button onClick={() => window.location.reload()} className="mt-8 rounded-full">Re-Initialize Vault</Button></div>;
