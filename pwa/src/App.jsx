    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter">
          ...
          <p className="text-text-dim text-sm mt-4 mb-10 max-w-xs font-bold opacity-60">{this.state.error?.message || "Something went wrong."}</p>
          <Button onClick={() => { localStorage.clear(); window.location.reload(); }} className="w-64 h-16 rounded-full shadow-2xl">Restart App</Button>
        </div>
      );
    }
...
const App = () => {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (u) => {
      ...
      setAppError("Login failed: " + err.message);
      setAuthLoading(false);
    });
    return () => unsub();
  }, []);
  ...
  function generateCoach(count, limit, streak) {
    ...
    if (count === 0 && streak > 0) return `You're on a ${streak}-day streak!`;
    if (p >= 1.0) return "Daily limit reached.";
    return "Track every session to see your progress.";
  }
  ...
  return (
    ...
    >
            ...
            {showProfileMenu && (
              <>
                ...
                <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }} className={cn("absolute right-0 mt-4 w-56 rounded-[32px] p-2 shadow-2xl border backdrop-blur-3xl z-[100]", settings.isDark ? "bg-black/90 border-white/10" : "bg-white/95 border-black/5 shadow-black/10 shadow-2xl")}>
                  ...
                  <button onClick={() => { signOut(auth); setShowProfileMenu(false); }} className="w-full flex items-center space-x-4 p-4 rounded-2xl hover:bg-danger/10 text-danger transition-colors text-sm font-[1000] uppercase tracking-widest border-t border-white/5 mt-1"><LogOut size={18} /><span>Log Out</span></button>
                </motion.div>
              </>
            )}
          </AnimatePresence>
      ...
      <AnimatePresence>
        ...
        {editTarget && <Overlay onClose={() => setEditTarget(null)} title="Edit Entry" isDark={settings.isDark}><EditForm log={editTarget} configs={configs} onSave={async (d, c) => { await setDoc(doc(db, 'users', user.uid, 'logs', d), { counts: c }, { merge: true }); setEditTarget(null); }} isDark={settings.isDark} /></Overlay>}
      </AnimatePresence>
    </div>
  );
};
    ...
    <div className="min-h-screen bg-[#020202] flex items-center justify-center p-6 text-white font-inter">
      <div className="w-full max-w-md space-y-12">
        ...
        <Card className="space-y-6 bg-white/[0.03] border-white/5 p-10 backdrop-blur-lg shadow-2xl shadow-black"><div className="flex flex-col space-y-6">
          ...
          {!isL && <Input label="Name" value={n} onChange={setN} placeholder="Your name" isDark={true} />}
          <Input label="Email" value={e} onChange={setE} placeholder="email@address.com" isDark={true} />
          <Input label="Password" type="password" value={p} onChange={setP} placeholder="••••••••" isDark={true} />
          <Button className="w-full h-18 text-xs shadow-2xl font-black uppercase tracking-[0.2em]" onClick={handle} disabled={loading} style={{'--accent': accent}}>{loading ? <Loader2 className="animate-spin" /> : (isL ? 'Sign In' : 'Create Account')}</Button>
          <div className="flex flex-col space-y-4 items-center pt-2"><button onClick={() => setIsL(!isL)} className="text-[10px] font-black text-text-dim uppercase tracking-widest hover:text-accent transition-all underline underline-offset-8 decoration-white/10">{isL ? "Create an account" : "Back to Sign In"}</button><button onClick={() => signInAnonymously(auth)} className="text-[10px] font-black text-accent/60 uppercase tracking-widest hover:text-accent transition-all">Guest mode</button></div>
        </div></Card>
      </div>
    </div>
  );
...
const SettingsScreen = ({ c, u, s, onAdd, onUpd, onReo, onDel }) => {
  ...
  const [gl, setGl] = useState(s.goal || 'SAVE FOR VACATION');
  ...
  return (
    <div className="flex flex-col space-y-10 md:space-y-12 pb-40">
       <Card className={cn("p-10 md:p-14 relative overflow-hidden shadow-2xl", s.isDark ? "bg-white/[0.03] border-white/5 shadow-black" : "bg-white border-black/5 shadow-black/5")}><div className="flex items-center space-x-8 md:space-x-12 mb-12 md:mb-16 relative z-10"><div className="w-32 h-32 md:w-36 md:h-36 bg-accent rounded-[42px] md:rounded-[56px] border-2 border-accent/20 flex items-center justify-center text-6xl md:text-7xl font-[1000] text-bg-base shadow-2xl overflow-hidden shadow-accent/20 transition-all active:scale-95">{u.photoURL ? <img src={u.photoURL} className="w-full h-full object-cover" alt="p" /> : al.charAt(0)}</div><div className="space-y-3 md:space-y-4"><h4 className={cn("text-4xl md:text-5xl font-[1000] tracking-tighter uppercase leading-none transition-colors", !s.isDark && "text-[#1D1D1F]")}>{al}</h4><div className="flex items-center space-x-3 md:space-x-4"><div className="h-2 w-2 md:h-2.5 md:w-2.5 rounded-full bg-accent animate-pulse shadow-[0_0_15px_var(--accent)]" /><span className="text-[11px] md:text-[12px] font-black uppercase tracking-[0.6em] text-accent font-bold">Profile Settings</span></div></div></div><div className="space-y-10 md:space-y-12 relative z-10"><Input label="Display Name" value={al} onChange={setAl} isDark={s.isDark} /><Input label="Your Goal" value={gl} onChange={setGl} isDark={s.isDark} /><Button className="w-full h-22 rounded-full shadow-2xl text-[12px] font-[1000] uppercase tracking-widest active:scale-95 transition-all" onClick={() => onUpd({ displayName: al, goal: gl })}>Update Profile</Button></div></Card>
       <Card className={cn("p-10 md:p-14 space-y-12 md:space-y-16 shadow-2xl", s.isDark ? "bg-white/[0.03] border-white/5 shadow-black" : "bg-white border-black/5 shadow-black/5 shadow-2xl")}>
          <div className="flex flex-col space-y-10"><div className="text-[12px] font-black uppercase tracking-[0.6em] opacity-40 mb-2">Display Preferences</div><Toggle icon={Moon} label="Dark Mode" active={ld} onClick={() => setLd(!ld)} isDark={s.isDark} /><Toggle icon={Clock} label="Night Owl Mode" active={s.nightOwl} onClick={() => onUpd({ nightOwl: !s.nightOwl })} isDark={s.isDark} /><Toggle icon={Grid} label="Compact Grid" active={s.layout === 'COMPACT'} onClick={() => onUpd({ layout: s.layout === 'LARGE' ? 'COMPACT' : 'LARGE' })} isDark={s.isDark} />
             <div className={cn("p-8 md:p-10 rounded-[36px] md:rounded-[48px] border space-y-10 shadow-inner transition-all mt-6", s.isDark ? "bg-black/40 border-white/5 shadow-black/40" : "bg-[#F0F2F5] border-black/5 shadow-inner")}><div className="flex items-center justify-between px-1 md:px-2 text-[10px] md:text-xs font-[1000] uppercase tracking-[0.5em] opacity-60"><span>Accent Color</span><Activity size={16} className="opacity-30" /></div><div className="flex flex-wrap gap-4 md:gap-5 justify-center">{ACCENTS.map(x => (
...
const EditForm = ({ log, configs, onSave, isDark }) => {
  ...
  return (
    <div className="space-y-12 md:space-y-14">
       ...
       <Button size="lg" className="w-full h-20 md:h-24 rounded-[32px] md:rounded-[42px] shadow-2xl text-sm md:text-base font-[1000] transition-all hover:scale-[1.02] active:scale-95 shadow-black/50" onClick={() => onSave(log.logDate, c)}>Save Changes</Button>
    </div>
  );
};
...
const ErrorView = ({ msg }) => <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter"><AlertCircle className="text-danger mb-6" size={48} /><h2 className="text-2xl font-black uppercase tracking-tighter leading-none mb-2 text-white font-black font-inter">System Error</h2><p className="text-text-dim text-sm max-w-xs">{msg}</p><Button onClick={() => window.location.reload()} className="mt-8 rounded-full">Restart</Button></div>;
const LoadingView = () => <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center space-y-6 text-accent font-inter"><Loader2 className="animate-spin" size={48} /><span className="text-[10px] font-black tracking-[0.5em] uppercase text-accent font-black">Loading...</span></div>;
