    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter">
          ...
          <h2 className="text-3xl font-[950] uppercase tracking-tighter">System Failure</h2>
...
const App = () => {
  ...
  return (
    <ErrorBoundary>
    <div className={cn("flex flex-col min-h-screen bg-bg-base text-white font-inter transition-all duration-700", !settings.isDark && "invert-[0.9] hue-rotate-180")} style={{ '--accent': settings.accent, '--accent-rgb': hexToRgb(settings.accent), fontSize: `${settings.fontScale}rem` }}>
      <header className="fixed top-0 left-0 right-0 z-40 pt-[env(safe-area-inset-top)] px-8 md:px-12 py-8 bg-gradient-to-b from-black via-black/80 to-transparent flex justify-between items-center">
        ...
        <div className="flex items-center space-x-6">
           <div className="w-12 h-12 rounded-2xl bg-accent/10 border border-accent/20 flex items-center justify-center text-accent font-[950] text-xl overflow-hidden shadow-2xl">{user.photoURL ? <img src={user.photoURL} alt="p" className="w-full h-full object-cover" /> : user.displayName?.charAt(0) || 'U'}</div>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto pt-40 pb-40 px-6 max-w-6xl mx-auto w-full transition-all duration-500">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'settings' && <SettingsScreen key="s" c={configs} setC={(v) => setConfigs(v)} u={user} s={settings} onAdd={() => setShowAdd(true)} onUpd={(upd) => setDoc(doc(db, 'users', user.uid), upd, { merge: true })} onReo={async (id, dir) => {
...
const CounterCard = ({ config, count, onInc, onDec, isC }) => {
  ...
  // Android Style: White body shrinks from LEFT to RIGHT.
  // This means width = (1-p)*75% and right = 25% (anchored to filter).
  const bodyW = isL ? 0 : (1 - p) * 72;
  return (
       ...
       <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-12">
          {config.type === 'CIGARETTE' ? (
             <div className={cn("relative w-full h-14 rounded-full overflow-hidden border border-white/5 mb-16 shadow-[inset_0_4px_20px_rgba(0,0,0,0.8)] transition-all duration-1000", isL ? "bg-danger shadow-[0_0_60px_#F87171]" : "bg-bg-panel/95")}>
                {/* White body anchored at start of roach (right 28%), shrinks FROM LEFT */}
                <motion.div animate={{ width: `${bodyW}%` }} transition={{ type:'spring', stiffness:45, damping:14 }} className="absolute right-[28%] inset-y-0 bg-gradient-to-l from-white to-neutral-200 z-10 shadow-[-15px_0_30px_rgba(255,255,255,0.3)] rounded-l-sm" />
                {!isL && count > 0 && <motion.div animate={{ right: `calc(28% + ${bodyW}%)` }} transition={{ type:'spring', stiffness:45, damping:14 }} className="absolute inset-y-0 w-16 translate-x-1/2 z-20 flex items-center justify-center"><div className="absolute inset-0 bg-radial-gradient from-[#FF3D00] via-[#FF3D00]/20 to-transparent blur-2xl opacity-80" /><motion.div animate={{ scale:[0.9,1.4,0.95], x:[-2,2,-1], backgroundColor:['#FF3D00','#FFB74D','#FF3D00'] }} transition={{ repeat: Infinity, duration:0.08, ease:'linear' }} className="w-6 h-full rounded-full shadow-[0_0_40px_#FF3D00] border-x border-white/20" /></motion.div>}
                <motion.div animate={{ width: isL ? '0%' : '28%', opacity: isL ? 0 : 1, scaleX: isL ? 0.5 : 1 }} transition={{ duration: 1 }} className="absolute inset-y-0 right-0 bg-[#D97706] z-[11] shadow-2xl border-l border-black/30 origin-right transition-all duration-1000" />
