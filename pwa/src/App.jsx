...
// --- GLOBAL CONSTANTS ---
const APP_VERSION = "21.0.0-IOS-POLISH";
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen w-full bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter">
          ...
          <p className="text-white/40 text-sm mb-10 max-w-md font-bold leading-relaxed">{this.state.error?.toString() || "A critical JavaScript error occurred."}</p>
...
const LoadingView = () => (
  <div className="min-h-screen w-full bg-[#020202] flex flex-col items-center justify-center space-y-12 text-accent font-inter font-black">
    <Loader2 className="animate-spin" size={100} strokeWidth={3} /><span className="text-sm font-black tracking-[1.5em] uppercase text-accent animate-pulse ml-[1.2em]">Synchronizing Registry</span>
  </div>
);
...
const SmokingProgress = React.memo(({ count, limit, variant }) => {
  ...
  return (
    <div className={cn("relative h-11 rounded-full overflow-hidden border-2 transition-all duration-1000 flex items-center shadow-2xl", variant === 'KING' ? "w-64" : (variant === 'QUEEN' ? "w-48" : "w-56"), isL ? "bg-danger border-danger shadow-[0_0_50px_rgba(255,0,0,0.6)]" : "bg-white/[0.03] border-white/10")}>
...
const TrackerCard = React.memo(({ config, count, onInc, onDec, index }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.05, type: 'spring', damping: 15 }} className={cn("bg-white/[0.02] rounded-[56px] border-2 p-10 flex flex-col items-center justify-between min-h-[520px] transition-all duration-700 group relative overflow-hidden shadow-2xl font-inter", isL ? "border-danger/30 shadow-[0_0_60px_rgba(248,113,113,0.1)]" : "border-white/[0.03] hover:border-accent/20")}>
      ...
      <div className="w-full flex justify-between items-center pt-8 mt-auto relative z-10 px-4 font-inter">
        {/* REFACTORED: Tactile +/- buttons with 44px hit target and subtle overlays */}
        <button onClick={onDec} className="w-16 h-16 rounded-full bg-white/[0.08] border border-white/10 flex items-center justify-center text-white/30 hover:text-white active:scale-90 transition-all shadow-xl backdrop-blur-md"><Minus size={28} strokeWidth={3} /></button>
        <button onClick={onInc} className={cn("w-16 h-16 rounded-full flex items-center justify-center text-black active:scale-90 transition-all shadow-[0_20px_50px_var(--accent-rgb)] backdrop-blur-md", isL ? "bg-danger shadow-[0_20px_50px_rgba(248,113,113,0.6)]" : "bg-accent")} style={{'--accent-rgb': 'rgba(0,210,255,0.4)'}}><Plus size={28} strokeWidth={4} /></button>
      </div>
    </motion.div>
  );
});
  ...
  return (
    <header className="sticky top-0 z-[100] w-full backdrop-blur-md bg-black/70 border-b border-white/[0.03]" style={{ paddingTop: 'max(env(safe-area-inset-top), 1rem)', paddingBottom: '1.25rem' }}>
      <div className="max-w-7xl mx-auto flex justify-between items-center px-6">
        <div className="flex flex-col text-left"><div className="flex items-center gap-2.5"><div className="w-2.5 h-2.5 rounded-full bg-accent animate-pulse shadow-[0_0_12px_var(--accent)]" /><h1 className="text-2xl font-[1000] tracking-tighter uppercase leading-none font-inter font-black">TABAK<span className="text-accent">++</span></h1></div><span className="text-[10px] font-black text-white/30 tracking-[0.4em] uppercase ml-4.5 mt-1.5 opacity-60">Registry Portal</span></div>
...
const ProtocolListItem = React.memo(({ config, idx, total, onReo, onEdit, onDel }) => (
  <div className="flex flex-row justify-between items-center w-full p-6 md:p-8 bg-[#121316] rounded-[32px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl overflow-hidden min-h-[100px] gap-6 font-inter">
    {/* REFACTORED: Enforced strict flex-1 overflow-hidden to prevent button overlap */}
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 font-inter">
       ...
       <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
         ...
         <InsightCard icon={Wallet} label="Retained" val={`$${(m.savings || 0).toFixed(2)}`} sub="Capital Saved" color="text-emerald-400" />
         <InsightCard icon={Activity} label="Impact" val={`${Math.floor((m.lost || 0)/60)}H`} sub="Time Restored" color="text-rose-400" />
       </div>
...
const AppContent = () => {
  ...
  // FIX: Explicit modal resolution logic
    ...
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 max-w-3xl mx-auto font-inter text-left">
       ...
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] shadow-2xl font-inter">
          <div className="space-y-10">
             ...
             {/* REFACTORED: Tactile Apply Scheme button */}
