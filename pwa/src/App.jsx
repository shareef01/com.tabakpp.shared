...
const APP_VERSION = "17.5.0-IOS-POLISH";
...
/**
 * <ProtocolListItem />
 * REFACTORED: Perfect iPhone 15 Pro Max Row Layout.
...
const ProtocolListItem = ({ config, idx, total, onReo, onEdit, onDel }) => (
  <div className="flex flex-row justify-between items-center w-full p-6 md:p-8 bg-white/[0.03] rounded-[32px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl overflow-hidden min-h-[100px] gap-6">
    <div className="flex flex-row items-center gap-6 min-w-0 flex-1">
      ...
      <div className="flex flex-col gap-0.5 min-w-0 flex-1 text-left">
        <span className="text-xl md:text-2xl font-[1000] tracking-tight uppercase group-hover:text-white transition-colors truncate leading-tight">
          {config.name}
        </span>
        <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] leading-relaxed">
          Target: {config.limit}
        </span>
      </div>
    </div>
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/85 backdrop-blur-2xl">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className="bg-[#121318] border border-white/10 rounded-[56px] w-full max-w-lg p-10 flex flex-col shadow-2xl relative overflow-hidden font-inter" style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 2rem)' }}>
          ...
          <button onClick={onAction} className="w-full h-18 md:h-20 bg-accent text-black font-[1000] uppercase tracking-[0.4em] rounded-[24px] shadow-2xl active:scale-95 transition-all flex items-center justify-center px-6"><span className="whitespace-nowrap text-xs md:text-sm font-black">{actionLabel}</span></button>
        </motion.div>
      </div>
    )}
  </AnimatePresence>
            ...
            <motion.div key="track" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ type: 'spring', damping: 20, stiffness: 100 }} className="space-y-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                 ...
                 <button onClick={() => setShowAdd(true)} className="bg-white/[0.01] rounded-[48px] border-2 border-dashed border-white/[0.05] flex flex-col items-center justify-center space-y-5 hover:bg-accent/5 hover:border-accent/20 transition-all min-h-[520px] group shadow-2xl">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white/10 group-hover:text-accent transition-colors font-inter">Add Protocol</span>
                 </button>
              </div>
            </motion.div>
          )}
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-12 font-inter">
       ...
       <div className="space-y-8 pt-12">
          <h4 className="text-[10px] font-black text-white/20 tracking-[1em] uppercase px-4 text-left">Timeline Feed</h4>
          {logs.map((log, i) => (
