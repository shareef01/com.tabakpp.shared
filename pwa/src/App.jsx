...
// --- GLOBAL CONSTANTS ---
const APP_VERSION = "20.0.0-CORE-RECOVERY";
...
const ErrorView = ({ msg }) => (
  <div className="min-h-screen w-full bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter">
    ...
    <h2 className="text-5xl font-[1000] uppercase tracking-tighter leading-none mb-8">Sync Failure</h2>
    <p className="text-white/20 text-sm max-w-sm font-bold opacity-60 leading-relaxed uppercase tracking-widest mb-16">{msg}</p>
...
class ErrorBoundaryUI extends Component {
  ...
  componentDidCatch(error, info) { console.error("SYSTEM_FAULT:", error, info); }
  render() {
...
const AppContent = () => {
  ...
  useEffect(() => {
    ...
    try {
      const uid = user.uid;
      const cUnsub = onSnapshot(collection(db, 'users', uid, 'configs'), (s) => {
      ...
      }, (e) => setAppError(e.message));

      const lUnsub = onSnapshot(query(collection(db, 'users', uid, 'logs'), orderBy('logDate', 'desc'), limit(30)), (s) => {
        setLogs(s.docs.map(d => d.data()));
      }, (e) => setAppError(e.message));

      const sUnsub = onSnapshot(doc(db, 'users', uid), (s) => {
        if (s.exists()) {
          const d = s.data();
          setSettings(p => ({ ...p,
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'track' && (
            <motion.div key="track" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ type: 'spring', damping: 20, stiffness: 100 }} className="space-y-10">
              ...
              <div className="flex items-center justify-between px-2 mb-[-1.5rem]">
                 <h4 className="text-[10px] font-black text-white/20 tracking-[0.8em] uppercase font-inter">Master Registry</h4>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                 ...
                 <button onClick={() => setShowAdd(true)} className="bg-white/[0.01] rounded-[48px] border-2 border-dashed border-white/[0.05] flex flex-col items-center justify-center space-y-5 hover:bg-accent/5 hover:border-accent/20 transition-all min-h-[520px] group shadow-2xl font-inter">
          ...
          {activeTab === 'control' && (
            <SettingsScreen
              ...
              onDel={async (id) => { if(window.confirm(`Permanently terminate protocol?`)) try { await deleteDoc(doc(db, 'users', user.uid, 'configs', id)); } catch (e) { alert(e.message); } }}
            />
          )}
        </AnimatePresence>
      </main>
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 max-w-3xl mx-auto font-inter text-left">
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-12 shadow-2xl font-inter"><div className="flex flex-col items-center gap-10 font-inter">
          ...
          <div className="w-full space-y-10 font-inter"><div className="relative group font-inter"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Identity Label</span><input value={n} onChange={e=>setN(e.target.value)} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-lg font-[1000] focus:border-accent/40 outline-none transition-all font-inter shadow-inner" /></div><button onClick={() => updateProfile(auth.currentUser, { displayName: n })} className="w-full h-20 bg-white text-black font-black uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-4 font-inter">Commit Profile <ChevronRight size={24} /></button></div>
       </div></Card>
