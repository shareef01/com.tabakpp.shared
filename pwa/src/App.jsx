    ...
    >
      ...
      <main className="flex-1 overflow-y-auto pt-[calc(env(safe-area-inset-top)+6.5rem)] pb-[calc(env(safe-area-inset-bottom)+12rem)] px-5 max-w-7xl mx-auto w-full transition-all duration-500 overflow-x-hidden">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'control' && <SettingsScreen configs={configs} user={user} settings={settings} onAdd={() => setShowAdd(true)} onReo={onReorder} onEditP={setEditProtocol} onUpd={onUpdateSettings} />}
        </AnimatePresence>
      </main>
...
const TopBanner = ({ m }) => (
  <section className="bg-white/[0.02] rounded-[48px] p-10 border border-white/[0.03] relative overflow-hidden group shadow-2xl">
...
const NavBtn = ({ id, icon: Icon, label, active, onClick }) => (
  <button onClick={onClick} className="relative flex-1 py-3 flex flex-col items-center gap-1.5 group transition-all duration-500">
    ...
    <span className={cn("text-[9px] font-black uppercase tracking-[0.3em] transition-all duration-500", active ? "text-white opacity-100" : "text-white/20")}>{label}</span>
  </button>
);
...
const SettingsScreen = ({ configs, user, settings, onAdd, onReo, onEditP, onUpd }) => {
                ...
                <div key={c.id} className="flex items-center justify-between p-10 bg-white/[0.03] rounded-[48px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl">
                   <div className="flex items-center gap-10">
                      ...
                      <div className="flex flex-col gap-1.5">
                        ...
                        <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em]">Target: {c.limit}</span>
                      </div>
                   </div>
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-8 bg-black/80 backdrop-blur-xl font-inter">
       <div className="bg-[#121318] border border-white/10 rounded-[64px] w-full max-w-[560px] p-16 space-y-12 shadow-2xl">
          ...
          <div className="space-y-10">
            ...
            <div className="space-y-5">
               <span className="text-[10px] font-black uppercase tracking-[0.8em] text-white/20 ml-2">Override Visuals</span>
               <div className="grid grid-cols-2 gap-5">
                  {['CIGARETTE', 'SIMPLE', 'JOINT_KING', 'JOINT_QUEEN'].map(x => (
                     <button key={x} onClick={() => setT(x)} className={cn("h-18 rounded-[32px] border-2 font-black text-[12px] uppercase tracking-widest transition-all flex items-center justify-center gap-4", t === x ? "border-accent bg-accent/10 text-accent shadow-[0_0_30px_rgba(0,210,255,0.2)]" : "border-white/5 text-white/20 hover:border-white/10")}>
                        {x.includes('KING') && <Crown size={18} />} {x.replace('_',' ')}
