...
// --- GLOBAL CONSTANTS ---
const APP_VERSION = "19.0.2-LOADING-FIX";
...
const SettingsScreen = ({ configs, user, settings, onAdd, onReo, onEditP, onUpd, onDel }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 max-w-3xl mx-auto font-inter text-left">
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] space-y-12 shadow-2xl font-inter"><div className="flex flex-col items-center gap-10 font-inter"><div className="relative group cursor-pointer active:scale-95 transition-all" onClick={() => fileRef.current?.click()}><div className={cn("w-40 h-40 rounded-[48px] bg-accent/5 border-2 border-accent/20 flex items-center justify-center overflow-hidden shadow-2xl transition-all duration-700", loading && "animate-pulse opacity-50")}>{user?.photoURL ? <img src={user.photoURL} className="w-full h-full object-cover" alt="p" /> : <User size={64} className="text-accent" />}</div><div className="absolute -bottom-3 -right-3 w-14 h-14 bg-white text-black rounded-[22px] flex items-center justify-center shadow-2xl border-4 border-[#020202]"><Camera size={24} strokeWidth={2.5} /></div><input type="file" ref={fileRef} className="hidden" accept="image/*" onChange={handleUpload} /></div><div className="w-full space-y-10 font-inter"><div className="relative group font-inter"><span className="absolute left-6 top-1 text-[10px] font-black text-accent uppercase tracking-widest opacity-40 font-inter">Identity Label</span><input value={n} onChange={e=>setN(e.target.value)} className="w-full h-20 bg-white/[0.03] border border-white/5 rounded-[28px] px-8 pt-6 text-lg font-[1000] focus:border-accent/40 outline-none transition-all font-inter shadow-inner" /></div><button onClick={() => updateProfile(auth.currentUser, { displayName: n })} className="w-full h-20 bg-white text-black font-black uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-4 font-inter font-black">Commit Profile <ChevronRight size={24} /></button></div></div></Card>
