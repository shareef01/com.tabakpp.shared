...
// --- GLOBAL CONSTANTS ---
const APP_VERSION = "28.1.0-LEGIBILITY-FIX";
...
const ErrorView = ({ msg }) => (
  <div className="min-h-screen w-full bg-[#020202] flex flex-col items-center justify-center p-12 text-center text-white font-inter">
    ...
    <p className="text-white/60 text-sm max-w-sm font-bold leading-relaxed uppercase tracking-widest mb-16 font-inter">{msg}</p>
...
const TrackerCard = React.memo(({ config, count, onInc, onDec, index, globalSize = 'LARGE' }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.05, type: 'spring', damping: 15 }} className={cn("bg-white/[0.02] rounded-[56px] border-2 flex flex-col items-center justify-between transition-all duration-700 group relative overflow-hidden shadow-2xl font-inter", isLarge ? "min-h-[520px] p-10" : (isMedium ? "min-h-[420px] p-9" : "min-h-[260px] p-6"), isL ? "border-danger/30 shadow-[0_0_60px_rgba(248,113,113,0.1)]" : "border-white/[0.03] hover:border-accent/20")}>
      {/* LEGIBILITY FIX: Increased label opacity to 50% */}
      {!isSmall && <span className={cn("font-black text-white/50 tracking-[0.4em] uppercase relative z-10 truncate w-full text-center shrink-0", isLarge ? "text-[10px]" : "text-[8px]")}>Limit: {config.limit}</span>}
      <div className={cn("flex-1 w-full flex flex-col items-center justify-center relative z-10 min-h-0", isSmall ? "space-y-4" : "space-y-8")}>
        ...
        <div className="flex flex-col items-center text-center min-w-0 w-full px-2">
          ...
          <span className={cn("font-black tracking-[0.4em] uppercase transition-all duration-700 truncate w-full shrink-0", isSmall ? "text-[10px] mt-2" : "text-[12px] mt-4", isL ? "text-danger" : "text-accent opacity-60 group-hover:opacity-100")}>{config.name}</span>
        </div>
      </div>
...
const HeaderSizeControl = React.memo(({ value, onChange }) => {
  ...
  return (
    <div className="relative bg-white/[0.03] border border-white/[0.05] p-0.5 rounded-[18px] flex items-center shadow-inner overflow-hidden">
      {options.map((opt) => (
        <button key={opt.id} onClick={() => onChange({ widgetSize: opt.id })} className={cn("relative p-2.5 transition-all duration-500 z-10 rounded-[14px] flex items-center justify-center w-9 h-9", value === opt.id ? "text-zinc-950" : "text-white/50 hover:text-white/80")}>
  ...
  return (
    <header className="sticky top-0 z-[100] w-full backdrop-blur-md bg-black/70 border-b border-white/[0.03]" style={{ paddingTop: 'max(env(safe-area-inset-top), 1rem)', paddingBottom: '1.25rem' }}>
      <div className="max-w-7xl mx-auto flex justify-between items-center px-6 font-inter">
        <div className="flex flex-col text-left font-inter"><div className="flex items-center gap-2.5"><div className="w-2.5 h-2.5 rounded-full bg-accent animate-pulse shadow-[0_0_12px_var(--accent)]" /><h1 className="text-2xl font-[1000] tracking-tighter uppercase leading-none font-black font-inter whitespace-nowrap flex items-center">TABAK<span className="text-accent">++</span></h1></div><span className="text-[10px] font-black text-white/50 tracking-[0.4em] uppercase ml-4.5 mt-1.5 opacity-80 font-inter">Dashboard</span></div>
        <div className="flex items-center gap-6">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            <AnimatePresence>{isOpen && ( <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="absolute right-0 mt-4 w-56 bg-[#121316] border border-white/[0.05] rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-3xl p-3 overflow-hidden font-inter z-[110]"><div className="px-4 py-3 border-b border-white/[0.03] mb-2 font-inter"><span className="text-[10px] font-black text-white/50 uppercase tracking-[0.3em] block">Account</span><span className="text-sm font-bold text-white truncate block mt-1">{user?.displayName || 'User'}</span></div><button onClick={() => { onNavigate('control'); setIsOpen(false); }} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-[16px] text-white/80 hover:text-white hover:bg-white/[0.03] transition-all group text-left font-black uppercase tracking-widest text-[10px] font-inter"><Settings size={18} /> Settings</button><button onClick={handleLogout} className="w-full flex items-center gap-4 px-4 py-3.5 rounded-[16px] text-rose-500/80 hover:text-rose-500 hover:bg-rose-500/5 transition-all text-left font-black uppercase tracking-widest text-[10px] font-inter"><LogOut size={18} /> Log Out</button></motion.div> )}</AnimatePresence>
          </div>
        </div>
      </div>
    </header>
  ...
  <section className="bg-white/[0.02] rounded-[48px] p-10 border border-white/[0.03] relative overflow-hidden group shadow-2xl font-inter">
    ...
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 relative z-10 font-inter">
      <div className="space-y-3 text-left">
        <h3 className="text-[11px] font-[1000] text-white/50 tracking-[0.6em] uppercase">Daily Allowance</h3>
      ...
      <div className="flex flex-col md:items-end gap-3 font-inter">
        <div className="flex items-center gap-4">
          ...
          <span className="text-3xl font-[1000] text-white/50 tracking-tighter tabular-nums font-inter">{m.xp || 0} <span className="text-sm font-bold opacity-60 uppercase tracking-widest font-inter">XP</span></span>
        </div>
      </div>
    </div>
  ...
  <div className="flex flex-row justify-between items-center w-full p-6 md:p-8 bg-[#121316] rounded-[32px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl overflow-hidden min-h-[100px] gap-6 font-inter">
    <div className="flex flex-row items-center gap-6 min-w-0 flex-1 text-left overflow-hidden">
      <div className="flex flex-col gap-2 shrink-0 items-center justify-center py-2">
        <button onClick={() => onReo(config.id, 'up')} disabled={idx === 0} className="text-white/50 hover:text-accent disabled:opacity-0 transition-all p-1 active:scale-75"><ArrowUp size={22} strokeWidth={3} /></button>
        <button onClick={() => onReo(config.id, 'down')} disabled={idx === total - 1} className="text-white/50 hover:text-accent disabled:opacity-0 transition-all p-1 active:scale-75"><ArrowDown size={22} strokeWidth={3} /></button>
      </div>
    ...
    <div className="flex flex-row items-center gap-4 shrink-0 ml-2">
      <button onClick={() => onEdit(config)} className="w-11 h-11 rounded-[14px] bg-white/[0.1] border border-white/[0.05] flex items-center justify-center text-white/50 hover:text-accent active:scale-90 shadow-md transition-all"><Edit2 size={18} /></button>
      <button onClick={onDel} className="w-11 h-11 rounded-[14px] bg-white/[0.1] border border-white/[0.05] flex items-center justify-center text-white/50 hover:text-danger active:scale-90 shadow-md transition-all"><Trash2 size={18} /></button>
    </div>
  </div>
));
...
const InsightCard = React.memo(({ icon: Icon, label, val, sub, color }) => (
  <Card className="p-6 lg:p-8 bg-white/[0.02] border border-white/[0.03] flex flex-col items-center justify-center text-center shadow-2xl rounded-[40px] group hover:border-accent/20 transition-all duration-700 min-h-[200px] lg:min-h-[220px] font-inter">
     ...
     <span className="text-[10px] font-black text-white/60 uppercase tracking-[0.4em] font-inter group-hover:text-white/80">{sub}</span>
     <div className="mt-6 pt-6 border-t border-white/[0.05] w-full text-[9px] font-[1000] uppercase tracking-[0.8em] text-accent opacity-40 font-inter group-hover:opacity-60">{label}</div>
  </Card>
));
...
const NavBtn = React.memo(({ id, icon: Icon, label, active, onClick }) => (
  <button onClick={onClick} className="relative flex-1 py-3 flex flex-col items-center gap-1.5 group transition-all duration-500 font-inter">
    ...
    <Icon size={24} className={cn("transition-all duration-500", active ? "text-accent scale-110 drop-shadow-[0_0_100px_var(--accent-rgb)]" : "text-white/40 group-hover:text-white/60")} style={{'--accent-rgb': 'rgba(0,210,255,0.4)'}} strokeWidth={active ? 3 : 2} />
    <span className={cn("text-[9px] font-black uppercase tracking-[0.3em] transition-all duration-500 font-inter", active ? "text-white opacity-100" : "text-white/40 opacity-0 group-hover:opacity-60 translate-y-2 group-hover:translate-y-0 font-inter")}>{label}</span>
  </button>
));
  ...
  return (
    <div className="min-h-screen bg-[#020202] flex flex-col lg:flex-row items-stretch text-white font-inter overflow-hidden selection:bg-accent/30 relative">
      <div className="absolute top-10 left-10 lg:top-16 lg:left-16 z-[100] flex flex-col items-start font-inter pointer-events-none">
         ...
         <span className="text-[9px] font-black text-white/50 tracking-[1em] uppercase mt-4 block font-inter">Quit Control System</span>
      </div>
      ...
      <div className="hidden lg:flex flex-1 flex-col items-center justify-center bg-black relative border-r border-white/[0.03]">
         ...
         <div className="flex-1 w-full flex flex-col items-center justify-center gap-0">
            ...
            <div className="text-center pt-2">
               ...
               <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.6em] mt-2 font-inter">RECLAIM CONTROL OF YOUR LIFE.</p>
            </div>
         </div>
      </div>
        ...
        <div className="w-full h-full flex flex-col items-center justify-center space-y-16">
          <div className="bg-white/[0.01] border border-white/[0.08] p-12 lg:p-18 rounded-[80px] space-y-12 shadow-[0_50px_150px_rgba(0,0,0,1)] backdrop-blur-3xl relative overflow-hidden font-inter w-full max-w-[500px] text-center max-h-[90vh] overflow-y-auto scrollbar-hide">
             ...
             <div className="relative bg-black/60 border border-white/[0.05] p-1.5 rounded-full flex items-center h-18 w-full shadow-inner overflow-hidden font-inter">
                <button onClick={() => setMode('LOGIN')} className={cn("relative flex-1 h-full text-[12px] font-[1000] uppercase tracking-[0.2em] transition-all duration-500 z-20 font-inter", mode === 'LOGIN' ? "text-zinc-950" : "text-white/50")}>Sign In</button>
                <button onClick={() => setMode('REGISTER')} className={cn("relative flex-1 h-full text-[12px] font-[1000] uppercase tracking-[0.2em] transition-all duration-500 z-20 font-inter", mode === 'REGISTER' ? "text-zinc-950" : "text-white/50")}>Sign Up</button>
             ...
             <div className="space-y-12 relative z-10 text-center font-inter">
               ...
               <button className="w-full h-24 bg-accent text-zinc-950 font-[1000] uppercase tracking-[0.5em] rounded-[36px] active:scale-95 transition-all shadow-[0_40px_80px_rgba(212,255,50,0.4)] flex items-center justify-center text-sm font-inter" onClick={handle}>{loading ? <Loader2 className="animate-spin" size={24} /> : (mode === 'LOGIN' ? 'Sign In' : (mode === 'REGISTER' ? 'Sign Up' : 'Reset'))}</button>
             </div>

             <div className="flex flex-col items-center gap-6 pt-4 text-center font-inter">
               {mode === 'LOGIN' && <button onClick={() => setMode('RESET')} className="text-white/50 uppercase text-[10px] font-black tracking-widest hover:text-accent transition-colors font-inter">Forgot Password?</button>}
               {mode === 'RESET' && <button onClick={() => setMode('LOGIN')} className="text-white/50 uppercase text-[10px] font-black tracking-widest hover:text-white transition-colors font-inter">Back to Sign In</button>}
             </div>
          </div>
        </div>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 max-w-3xl mx-auto font-inter text-left font-inter">
       ...
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] shadow-2xl font-inter">
          <div className="space-y-10 font-inter"><div className="px-2 text-left font-inter"><h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/50 mb-2 font-inter">Appearance</h3><span className="text-3xl font-[1000] tracking-tighter uppercase font-inter font-black">Accent Spectrum</span></div><div className="grid grid-cols-3 gap-6 font-inter">{ACCENTS.map(x => ( <button key={x.v} onClick={() => setLa(x.v)} className={cn("h-16 rounded-[24px] border-2 transition-all duration-500 relative flex items-center justify-center font-inter", la === x.v ? "border-white scale-105 shadow-2xl" : "border-white/[0.05] opacity-40 hover:opacity-100")} style={{ backgroundColor: x.v }}>{la === x.v && <Check size={24} className="text-white drop-shadow-md" strokeWidth={4} />}</button> ))}</div><button onClick={() => onUpd({ accent: la })} className="w-full h-20 bg-white/[0.1] border border-white/5 text-white font-[1000] uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-xl hover:bg-white/[0.15] font-inter">Save Color</button></div></Card>
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] shadow-2xl font-inter">
