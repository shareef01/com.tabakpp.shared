...
const APP_VERSION = "16.0.0-IOS-NATIVE-FLUID";
  ...
  return (
    ...
    >
      ...
      {/*
        ...
        The main screen is now a single, fluid mobile scrollable view.
        Removed the 'Daily Goal' from this view entirely.
      */}
      <main className="flex-1 overflow-y-auto pt-10 pb-[calc(env(safe-area-inset-bottom)+12rem)] px-5 max-w-7xl mx-auto w-full transition-all duration-500 overflow-x-hidden">
        <AnimatePresence mode="wait">
          {activeTab === 'track' && (
            <motion.div key="track" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ type: 'spring', damping: 20, stiffness: 100 }} className="space-y-6">

              <div className="flex items-center justify-between px-2">
                 <h4 className="text-[10px] font-[1000] text-white/30 tracking-[0.6em] uppercase font-inter">Master Registry</h4>
              ...
              <div className="flex flex-col gap-4">
                 {configs.sort((a,b)=>a.order-b.order).map((c, i) => (
                   <IPhoneTrackingCard
                      ...
                      onReo={onReorder}
                      onEdit={setEditProtocol}
                      onDelete={async (id) => { if(window.confirm(`Permanently terminate this protocol?`)) await deleteDoc(doc(db, 'users', user.uid, 'configs', id)); }}
                      count={(metrics.todayLog.counts || {})[c.id] || 0}
                      onInc={() => onInc(c.id)}
                      onDec={() => onDec(c.id)}
                   />
                 ))}
                 <button onClick={() => setShowAdd(true)} className="bg-white/[0.01] rounded-[32px] border-2 border-dashed border-white/[0.05] flex flex-row items-center justify-center gap-4 py-8 hover:bg-accent/5 hover:border-accent/20 transition-all group shadow-2xl">
                    <div className="w-12 h-12 rounded-[18px] bg-white/[0.03] flex items-center justify-center text-white/10 group-hover:text-accent group-hover:bg-accent/10 transition-all border border-white/5"><Plus size={24} /></div>
                    <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white/10 group-hover:text-accent transition-colors font-inter">Add Registry Entry</span>
                 </button>
              </div>
            </motion.div>
          )}
          {activeTab === 'history' && <HistoryScreen logs={logs} configs={configs} m={metrics} onEdit={setEditTarget} userId={user.uid} today={today} />}
          {activeTab === 'control' && <SettingsScreen settings={settings} onUpd={onUpdateSettings} />}
        </AnimatePresence>
      </main>
...
// --- CORE UI COMPONENTS (REFACTORED FOR IOS NATIVE FEEL) ---
...
/**
 * <IPhoneTrackingCard />
 * REFACTORED: Strict native iOS Row Layout.
 * FIX: 'flex-1' text container prevents action buttons from overlapping titles.
 * FIX: Removed bulky arrow boxes for a cleaner 'HStack' feel.
 */
const IPhoneTrackingCard = ({ config, idx, total, onReo, onEdit, onDelete, count, onInc, onDec }) => (
  <div className="flex flex-row items-center w-full p-6 bg-white/[0.03] rounded-[32px] border border-white/[0.05] group hover:border-accent/20 transition-all duration-500 shadow-xl overflow-hidden min-h-[110px] gap-4">
    ...
    {/* Component 2: Main Icon (Center-Left) - Perfectly centered */}
    <div className="w-16 h-16 bg-white/[0.03] border border-white/5 rounded-[22px] flex items-center justify-center text-accent group-hover:text-accent transition-all shrink-0 shadow-inner">
      {config.type.startsWith('JOINT') ? <Crown size={32} /> : (config.type === 'SIMPLE' ? <Activity size={32} /> : <Zap size={32} />)}
    </div>

    {/* Component 3: Text Container (Center) - FLEX-GROW forces alignment fix */}
    <div className="flex flex-col gap-0.5 min-w-0 flex-1 ml-2">
      <span className="text-2xl font-[1000] tracking-tight uppercase group-hover:text-white transition-colors truncate">{config.name}</span>
      ...
      <div className="flex items-center gap-4 mt-2">
        <span className="text-3xl font-[1000] text-accent tabular-nums">{count}</span>
        <div className="flex items-center gap-3">
          <button onClick={onDec} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-white/30 hover:text-white active:scale-75"><Minus size={16} /></button>
          <button onClick={onInc} className="w-8 h-8 rounded-lg bg-accent flex items-center justify-center text-black active:scale-75 shadow-lg shadow-accent/20"><Plus size={16} /></button>
        </div>
      </div>
    </div>

    {/* Component 4: Action Buttons (Far Right) - Anchored to edge */}
    <div className="flex flex-row items-center gap-4 ml-4 shrink-0">
      <button
        onClick={() => onEdit(config)}
        className="w-11 h-11 rounded-[14px] bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/20 hover:text-accent hover:border-accent/20 transition-all shadow-md active:scale-90"
      >
        <Edit2 size={20} />
      </button>
      <button
        onClick={() => onDelete(config.id)}
        className="w-11 h-11 rounded-[14px] bg-white/[0.03] border border-white/[0.05] flex items-center justify-center text-white/20 hover:text-danger hover:border-danger/20 transition-all shadow-md active:scale-90"
      >
        <Trash2 size={20} />
      </button>
    </div>
...
/**
 * <IPhoneModifyModal />
 * REBUILT: Production-ready modal with Safari-specific safe-area support.
 */
    ...
    {isOpen && (
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/85 backdrop-blur-2xl">
        ...
        >
          ...
          >
            <span className="whitespace-nowrap text-sm">{actionLabel}</span>
          </button>
        </motion.div>
      </div>
    )}
...
const SettingsScreen = ({ settings, onUpd }) => {
