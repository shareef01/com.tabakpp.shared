...
// --- GLOBAL CONSTANTS ---
const APP_VERSION = "28.5.0-LOGOUT-REFINED";
...
const TopBanner = React.memo(({ user, onNavigate, widgetSize, onUpdateSettings, onRequestLogout }) => {
  ...
  <div className="flex flex-row justify-between items-center w-full p-6 md:p-8 bg-[#121316] rounded-[32px] border border-white/[0.05] group hover:border-accent/30 transition-all duration-500 shadow-xl overflow-hidden min-h-[100px] gap-6 font-inter">
    <div className="flex flex-row items-center gap-6 min-w-0 flex-1 text-left overflow-hidden">
      <div className="flex flex-col gap-2 shrink-0 items-center justify-center py-2">
        <button onClick={() => onReo(config.id, 'up')} disabled={idx === 0} className="text-white/70 hover:text-accent disabled:opacity-0 transition-all p-1 active:scale-75"><ArrowUp size={22} strokeWidth={3} /></button>
        <button onClick={() => onReo(config.id, 'down')} disabled={idx === total - 1} className="text-white/70 hover:text-accent disabled:opacity-0 transition-all p-1 active:scale-75"><ArrowDown size={22} strokeWidth={3} /></button>
      </div>
    ...
    <div className="flex flex-row items-center gap-4 shrink-0 ml-2">
      <button onClick={() => onEdit(config)} className="w-11 h-11 rounded-[14px] bg-white/[0.1] border border-white/[0.05] flex items-center justify-center text-white/70 hover:text-accent active:scale-90 shadow-md transition-all"><Edit2 size={18} /></button>
      <button onClick={onDel} className="w-11 h-11 rounded-[14px] bg-white/[0.1] border border-white/[0.05] flex items-center justify-center text-white/70 hover:text-danger active:scale-90 shadow-md transition-all"><Trash2 size={18} /></button>
    </div>
  </div>
));
        ...
        <div className="w-full h-full flex flex-col items-center justify-center space-y-16">
          <div className="bg-white/[0.01] border border-white/[0.08] p-12 lg:p-18 rounded-[80px] space-y-12 shadow-[0_50px_150px_rgba(0,0,0,1)] backdrop-blur-3xl relative overflow-hidden font-inter w-full text-center max-h-[90vh] overflow-y-auto scrollbar-hide">
             ...
             <div className="relative bg-black/60 border border-white/[0.05] p-1.5 rounded-full flex items-center h-18 w-full shadow-inner overflow-hidden font-inter">
                <button onClick={() => setMode('LOGIN')} className={cn("relative flex-1 h-full text-[12px] font-[1000] uppercase tracking-[0.2em] transition-all duration-500 z-20 font-inter", mode === 'LOGIN' ? "text-zinc-950" : "text-white/70")}>Sign In</button>
                <button onClick={() => setMode('REGISTER')} className={cn("relative flex-1 h-full text-[12px] font-[1000] uppercase tracking-[0.2em] transition-all duration-500 z-20 font-inter", mode === 'REGISTER' ? "text-zinc-950" : "text-white/70")}>Sign Up</button>
                <motion.div
             ...
             <div className="flex flex-col items-center gap-6 pt-4 text-center font-inter">
               {mode === 'LOGIN' && <button onClick={() => setMode('RESET')} className="text-white/70 uppercase text-[10px] font-black tracking-widest hover:text-accent transition-colors font-inter">Forgot Password?</button>}
               {mode === 'RESET' && <button onClick={() => setMode('LOGIN')} className="text-white/70 uppercase text-[10px] font-black tracking-widest hover:text-white transition-colors font-inter">Back to Sign In</button>}
             </div>
          </div>
        </div>
      </div>
  ...
  <AnimatePresence>
    {isOpen && (
      <div className="fixed inset-0 z-[300] flex items-center justify-center p-8 bg-black/85 backdrop-blur-2xl font-inter">
        <motion.div initial={{ opacity: 0, scale: 0.9, y: 100 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 100 }} className="bg-[#121318] border border-white/10 rounded-[64px] w-full max-w-[440px] p-16 flex flex-col shadow-2xl relative overflow-hidden text-center">
           ...
           <div className="relative z-10 flex flex-col items-center space-y-10">
              ...
              <div className="space-y-4">
                 ...
                 <p className="text-white/60 text-xs font-black uppercase tracking-widest leading-relaxed">You will be required to re-authenticate to access your registry.</p>
              </div>
              <div className="flex flex-col w-full gap-5">
                 <button onClick={onConfirm} className="w-full h-20 bg-rose-500 text-zinc-950 font-[1000] uppercase tracking-[0.4em] rounded-[28px] shadow-2xl active:scale-95 transition-all">Terminate</button>
                 <button onClick={onClose} className="w-full h-20 bg-white/[0.05] border border-white/[0.08] text-white/70 font-[1000] uppercase tracking-[0.4em] rounded-[28px] hover:bg-white/[0.08] transition-all">Keep Syncing</button>
              </div>
           </div>
        </motion.div>
      </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="space-y-12 max-w-3xl mx-auto font-inter text-left font-inter">
       ...
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] shadow-2xl font-inter">
          <div className="space-y-10 font-inter"><div className="px-2 text-left font-inter"><h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/70 mb-2 font-inter">Appearance</h3><span className="text-3xl font-[1000] tracking-tighter uppercase font-inter font-black">Accent Spectrum</span></div><div className="grid grid-cols-3 gap-6 font-inter">{ACCENTS.map(x => ( <button key={x.v} onClick={() => setLa(x.v)} className={cn("h-16 rounded-[24px] border-2 transition-all duration-500 relative flex items-center justify-center font-inter", la === x.v ? "border-white scale-105 shadow-2xl" : "border-white/[0.05] opacity-40 hover:opacity-100")} style={{ backgroundColor: x.v }}>{la === x.v && <Check size={24} className="text-white drop-shadow-md" strokeWidth={4} />}</button> ))}</div><button onClick={() => onUpd({ accent: la })} className="w-full h-20 bg-white/[0.1] border border-white/5 text-white font-[1000] uppercase tracking-[0.5em] rounded-[28px] active:scale-95 transition-all shadow-xl hover:bg-white/[0.15] font-inter">Save Color</button></div></Card>
       <Card className="p-12 bg-white/[0.02] border border-white/[0.03] rounded-[56px] shadow-2xl font-inter">
          <div className="flex items-center justify-between gap-8 mb-10 px-2 font-inter">
             <div className="space-y-2 text-left font-inter min-w-0 flex-1">
                <h3 className="text-[10px] font-black uppercase tracking-[0.8em] text-white/70 font-inter">Management</h3>
