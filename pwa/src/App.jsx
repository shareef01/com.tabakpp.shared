...
import {
  ...
  Database, ShieldCheck, Flame, Loader2, InfoIcon
} from 'lucide-react';
import {
...
const App = () => {
  ...
  function generateCoach(count, limit, streak) {
    ...
    if (p >= 1.0) return "Limit reached. Critical capacity confirmed.";
    if (p > 0.8) return "Nearing threshold. Exercise protocol restraint.";
    return "Every session tracked strengthens the vault integrity.";
  }
  ...
  const onInc = async (id) => {
    ...
    await setDoc(ref, { logDate: today, counts: { ...counts, [id]: cnt + 1 } }, { merge: true });
  };
  ...
  const onDec = async (id) => {
    ...
    if (cnt <= 0) return;
    const ref = doc(db, 'users', user.uid, 'logs', today);
    await setDoc(ref, { counts: { ...counts, [id]: cnt - 1 } }, { merge: true });
  };
    ...
    <div className={cn("flex flex-col min-h-screen bg-bg-base text-white font-inter selection:bg-accent/30 transition-all duration-700", !settings.isDark && "invert-[0.9] hue-rotate-180")} style={{ '--accent': settings.accent, '--accent-rgb': hexToRgb(settings.accent), fontSize: `${settings.fontScale}rem` }}>
      <header className="fixed top-0 left-0 right-0 z-40 pt-[env(safe-area-inset-top)] px-8 md:px-12 py-8 bg-gradient-to-b from-black via-black/80 to-transparent flex justify-between items-center">
        <div className="flex flex-col">
          ...
          <div className="flex items-center space-x-2 mt-2"><div className="h-1.5 w-1.5 rounded-full bg-accent animate-pulse" /><span className="text-[10px] font-[900] tracking-[0.4em] text-accent uppercase leading-none">{activeTab}</span></div>
        </div>
...
const TrackerScreen = ({ m, c, onInc, onDec, view, onAdd }) => (
  <div className="flex flex-col space-y-12 pb-10">
    <StaggeredItem index={0}>
       <Card className="bg-bg-panel/40 border-accent/10 p-12 relative overflow-hidden group shadow-2xl">
          <div className="flex justify-between items-end relative z-10 text-white">
             <div className="space-y-2">
                <span className="text-[10px] font-[900] text-text-dim uppercase tracking-[0.4em]">Vault Integrity</span>
                <div className="text-7xl font-[950] tracking-tighter leading-none">{Math.max(0, m.limit - m.count)} <span className="text-base text-accent uppercase tracking-widest font-black ml-4 leading-none">Remaining</span></div>
             </div>
             <div className="text-right space-y-3 hidden md:block">
                ...
                <div className="text-4xl font-[950] text-text-muted leading-none tracking-tighter">{m.xp} XP</div>
             </div>
          </div>
          <div className="w-full h-3.5 bg-white/5 rounded-full overflow-hidden mt-12 p-0.5 border border-white/5 relative z-10">
             <motion.div animate={{ width: `${Math.min(1, m.progress) * 100}%` }} className={cn("h-full rounded-full transition-all duration-1000", m.progress >= 1 ? "bg-danger shadow-[0_0_30px_#F87171]" : "bg-white shadow-[0_0_20px_white]")} />
          </div>
          <div className="p-6 bg-white/[0.03] rounded-[32px] border border-white/5 flex items-start space-x-5 mt-10 relative z-10 backdrop-blur-sm">
             <div className="p-3 bg-accent/10 rounded-2xl text-accent"><InfoIcon size={20} /></div>
             <p className="text-sm font-bold text-text-muted leading-relaxed italic opacity-80">"{m.coach}"</p>
          </div>
    ...
    <div className={cn("grid gap-10", view === 'COMPACT' ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-1 lg:grid-cols-2")}>
       ...
       <StaggeredItem index={c.length + 1}>
          <button onClick={onAdd} className={cn("w-full border-2 border-dashed border-white/5 rounded-[48px] flex flex-col items-center justify-center space-y-6 hover:bg-white/[0.02] hover:border-accent/20 transition-all group relative overflow-hidden", view === 'COMPACT' ? "h-[350px]" : "h-[500px]")}>
             <div className="w-20 h-20 rounded-[32px] bg-white/5 flex items-center justify-center group-hover:scale-110 group-hover:bg-accent/10 transition-all duration-500 shadow-inner"><Plus className="text-text-dim group-hover:text-accent" size={32} /></div>
...
const CounterCard = ({ config, count, onInc, onDec, isC }) => {
  ...
  // Design logic matching screenshot: Fills Left to Right.
  // Ash/Spent part on left, white body on right.
  const ashW = isL ? 100 : p * 100;
  const bodyW = 100 - ashW;
  ...
  return (
    <Card className={cn("relative flex flex-col group transition-all duration-1000 p-12 overflow-hidden shadow-2xl", isL ? "bg-danger/[0.04] border-danger/50 shadow-[0_0_60px_rgba(248,113,113,0.15)]" : "hover:border-accent/30 border-white/5", isC ? "min-h-[380px]" : "min-h-[550px]")}>
       <div className="flex justify-between items-start mb-10 relative z-20 text-center flex-col items-center w-full">
          <span className={cn("text-[12px] font-[950] tracking-[0.5em] uppercase transition-all duration-700", isL ? "text-danger" : "text-accent")}>{config.name}</span>
          <span className="text-[9px] font-black text-text-dim uppercase tracking-[0.2em] opacity-60 mt-1">Daily Target: {config.limit}</span>
       </div>

       <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-12">
          {(isCig || isJoint) ? (
             <div className={cn("relative w-full h-12 rounded-full overflow-hidden border border-white/5 mb-16 shadow-[inset_0_4px_20px_rgba(0,0,0,0.8)] transition-all duration-1000", isL ? "bg-danger shadow-[0_0_60px_#F87171]" : "bg-bg-panel/95")}>
                {/* Ash / Spent Part (Grows from Left) */}
                <motion.div
                   animate={{ width: `${ashW}%` }}
                   className={cn("absolute left-0 inset-y-0 bg-[#1a1a1a] transition-colors duration-1000", isL && "bg-danger/20")}
                />

                {/* White body (Remains on Right) */}
                <motion.div
                  animate={{ left: `${ashW}%` }}
                  className={cn("absolute right-[28%] inset-y-0 z-10 transition-all duration-700", isCig ? "bg-white shadow-[0_0_30px_rgba(255,255,255,0.3)]" : "bg-[#C8E6C9] shadow-[0_0_30px_rgba(200,230,201,0.3)]", isL && "opacity-0")}
                />

                {/* Active Ember (Tracks the edge) */}
                {!isL && count > 0 && (
                  <motion.div animate={{ left: `${ashW}%` }} className="absolute inset-y-0 w-16 -translate-x-1/2 z-20 flex items-center justify-center pointer-events-none">
                    <div className="absolute inset-0 bg-radial-gradient from-[#FF3D00] via-[#FF3D00]/20 to-transparent blur-3xl opacity-90" />
                    <motion.div animate={{ scale:[1,1.4,1], opacity:[0.8,1,0.8], x:[-1,2,0] }} transition={{ repeat: Infinity, duration:0.1 }} className="w-6 h-full bg-[#FF3D00] rounded-full shadow-[0_0_40px_#FF3D00] border-x border-white/20" />
                  </motion.div>
                )}

                {/* Filter / Roach (Fixed on Right) */}
                <motion.div
                   animate={{ opacity: isL ? 0.3 : 1, width: isL ? '10%' : '28%' }}
                   className={cn("absolute inset-y-0 right-0 z-[11] shadow-2xl border-l border-black/30 origin-right transition-all duration-1000", isCig ? "bg-[#D97706]" : "bg-[#333]")}
                />
             </div>
          ) : (
          ...
          <motion.div key={count} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type:'spring', stiffness:100 }} className={cn("font-[1000] tracking-[-0.1em] leading-none mb-10 transition-all text-white shadow-black drop-shadow-2xl", isC ? "text-8xl" : "text-[12rem]", isL && "text-danger")}>{count}</motion.div>
       </div>

       <div className={cn("flex justify-center items-center relative z-20", isC ? "space-x-8" : "space-x-12")}>
          <motion.button whileTap={{ scale: 0.8 }} onClick={() => onDec(config.id)} className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-text-muted hover:text-white transition-all shadow-xl hover:bg-white/10 hover:scale-110"><Minus size={32} /></motion.button>
          <motion.button whileTap={{ scale: 0.9 }} onClick={() => onInc(config.id)} className={cn("w-20 h-20 rounded-full border-2 flex items-center justify-center transition-all shadow-[0_20px_50px_rgba(0,0,0,0.6)]", isL ? "border-danger text-danger bg-danger/10" : "border-accent/60 text-accent bg-accent/5 hover:border-accent hover:rotate-1")}><Plus size={32} /></motion.button>
       </div>
    </Card>
  );
};
