...
import {
  ...
  Moon, Check, GripVertical, CheckCircle2, Loader2, InfoIcon, Flame
} from 'lucide-react';
import {
...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-bg-base flex flex-col items-center justify-center p-12 text-center text-white">
          <AlertCircle size={48} className="text-danger mb-6" />
          <h2 className="text-2xl font-black uppercase">System Failure</h2>
          <p className="text-text-dim text-sm mt-2 mb-8 max-w-xs">{this.state.error?.message}</p>
  ...
  return (
    ...
    <div className={cn("flex flex-col min-h-screen bg-bg-base text-white font-inter selection:bg-accent/30 transition-all duration-700", !settings.isDark && "invert-[0.9] hue-rotate-180")} style={{ '--accent': settings.accent, '--accent-rgb': hexToRgb(settings.accent), fontSize: `${settings.fontScale}rem` }}>
      ...
      <AnimatePresence>
        ...
        {editTarget && <Overlay onClose={() => setEditTarget(null)} title="Override Data"><EditForm log={editTarget} configs={configs} onSave={async (d, c) => { await setDoc(doc(db, 'users', user.uid, 'logs', d), { counts: c }, { merge: true }); setEditTarget(null); }} /></Overlay>}
      </AnimatePresence>
    </div>
    </ErrorBoundary>
  );
  ...
  return (
    <div className="min-h-screen bg-bg-base flex items-center justify-center p-6 text-white font-inter">
      <div className="w-full max-w-md space-y-12 text-center">
        <div className="flex flex-col items-center"><div className="w-24 h-24 bg-accent rounded-[32px] flex items-center justify-center mb-8 shadow-2xl text-bg-base"><LayoutDashboard size={40} /></div><h1 className="text-5xl font-[950] tracking-tighter uppercase leading-none">tabak++</h1><p className="text-accent text-[10px] font-black tracking-[0.5em] mt-4 uppercase opacity-60">Neural Link Authorization</p></div>
        <Card className="space-y-6 bg-bg-panel/50 border-white/5 p-10 backdrop-blur-lg shadow-black/80 shadow-2xl">
          ...
          <div className="flex flex-col space-y-4 items-center pt-2">
             <button onClick={() => setIsL(!isL)} className="text-[10px] font-black text-text-dim uppercase tracking-widest hover:text-accent transition-all">{isL ? "Request New Vault" : "Return to Login"}</button>
...
const CounterCard = ({ config, count, onInc, onDec, isC }) => {
  ...
  return (
    <Card className={cn("relative flex flex-col group transition-all duration-1000 p-12 overflow-hidden", isL ? "bg-danger/[0.04] border-danger/50 shadow-[0_0_60px_rgba(248,113,113,0.15)]" : "hover:border-accent/30 border-white/5 shadow-2xl", isC ? "min-h-[380px]" : "min-h-[550px]")}>
       <div className="flex justify-between items-start mb-10 relative z-20 text-white"><div className="flex flex-col space-y-1"><span className={cn("text-[11px] font-[950] tracking-[0.5em] uppercase transition-all duration-700", isL ? "text-danger" : "text-accent")}>{config.name}</span>{!isC && <span className="text-[10px] font-black text-text-dim uppercase tracking-[0.2em] opacity-60">Threshold: {config.limit} units</span>}</div>{isL && <motion.div animate={{ scale:[1,1.2,1], opacity:[1,0.5,1] }} transition={{ repeat: Infinity, duration: 0.8 }} className="text-danger p-3 bg-danger/10 rounded-2xl border border-danger/20"><AlertCircle size={20} /></motion.div>}</div>
       <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-12">
          {config.type === 'CIGARETTE' ? (
             <div className={cn("relative w-full h-14 rounded-full overflow-hidden border border-white/5 mb-16 shadow-[inset_0_4px_20px_rgba(0,0,0,0.8)] transition-all duration-1000", isL ? "bg-danger shadow-[0_0_60px_#F87171]" : "bg-bg-panel")}>
                {/* White body anchored at start of roach, shrinks FROM LEFT */}
                <motion.div animate={{ width: `${bodyW}%` }} transition={{ type:'spring', stiffness:60, damping:16 }} className="absolute right-0 inset-y-0 bg-gradient-to-l from-white to-neutral-200 z-10 shadow-[-15px_0_30px_rgba(255,255,255,0.3)]" />
                {!isL && count > 0 && <motion.div animate={{ right: `${bodyW}%` }} transition={{ type:'spring', stiffness:60, damping:16 }} className="absolute inset-y-0 w-16 translate-x-1/2 z-20 flex items-center justify-center"><div className="absolute inset-0 bg-radial-gradient from-[#FF3D00] via-[#FF3D00]/20 to-transparent blur-2xl opacity-80" /><motion.div animate={{ scale:[0.9,1.4,0.95], x:[-2,2,-1], backgroundColor:['#FF3D00','#FFB74D','#FF3D00'] }} transition={{ repeat: Infinity, duration:0.08, ease:'linear' }} className="w-6 h-full rounded-full shadow-[0_0_40px_#FF3D00] border-x border-white/20" /></motion.div>}
                <motion.div animate={{ width: isL ? '0%' : '30%', opacity: isL ? 0 : 1 }} transition={{ duration: 1 }} className="absolute inset-y-0 right-0 bg-[#D97706] z-[11] shadow-2xl border-l border-black/30" />
                <div className="absolute inset-0 bg-neutral-950/90 -z-10" />
             </div>
          ) : <div className={cn("text-white font-[950] tracking-tighter leading-none mb-10 transition-all", isC ? "text-8xl" : "text-[12rem]", isL ? "text-danger drop-shadow-[0_0_30px_rgba(248,113,113,0.4)]" : "text-white")}>{count}</div>}
          {config.type === 'CIGARETTE' && <div className={cn("font-[950] tracking-tighter leading-none mb-10 text-white", isC ? "text-7xl" : "text-[11rem]")}>{count}</div>}
       </div>
...
const NavItem = ({ icon: Icon, active, onClick, label }) => (
  <motion.div onClick={onClick} whileTap={{ scale: 0.8 }} className="flex flex-col items-center justify-center flex-1 py-5 cursor-pointer relative group text-white">
    <Icon size={28} className={cn("mb-3 transition-all duration-1000", active ? "text-accent scale-125 drop-shadow-[0_0_20px_#D4FF5C]" : "text-text-dim group-hover:text-text-muted")} /><span className={cn("text-[11px] font-black tracking-[0.3em] uppercase transition-colors duration-500", active ? "text-white" : "text-text-dim")}>{label}</span>
    {active && <motion.div layoutId="navDot" className="absolute -top-1 w-2.5 h-2.5 rounded-full bg-accent shadow-[0_0_30px_#D4FF5C]" />}
  </motion.div>
);
...
const Toggle = ({ icon: Icon, label, active, onClick }) => (
  <div className="flex items-center justify-between p-10 bg-white/[0.02] rounded-[48px] border border-white/5 shadow-2xl transition-all hover:bg-white/[0.04] group text-white">
     ...
     <button onClick={onClick} className={cn("w-20 h-11 rounded-full p-2.5 transition-all duration-1000 border border-white/10 shadow-inner shadow-black/50", active ? "bg-accent shadow-accent/50" : "bg-white/10")}><div className={cn("w-6 h-6 rounded-full bg-white transition-all duration-1000 shadow-2xl", active ? "translate-x-10" : "translate-x-0")} /></button>
  </div>
);
...
const Overlay = ({ children, onClose, title }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/95 backdrop-blur-3xl overflow-y-auto">
     <motion.div initial={{ y: 200, scale: 0.95, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} exit={{ y: 200, scale: 0.95, opacity: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 120 }} className="w-full max-w-2xl my-auto">
        <Card className="p-16 relative border-accent/40 shadow-[0_0_200px_rgba(212,255,92,0.2)] bg-bg-panel/98 rounded-[64px] border-2 shadow-black/80"><button onClick={onClose} className="absolute top-14 right-14 p-6 rounded-[32px] bg-white/5 text-text-dim hover:text-white transition-all border border-white/5 shadow-2xl group"><X size={32} className="group-hover:rotate-90 transition-transform duration-700" /></button><div className="flex items-center space-x-8 text-accent mb-20 border-b border-white/10 pb-12"><div className="p-6 bg-accent/10 rounded-[32px] border border-accent/20 shadow-2xl text-accent"><Activity size={40} className="animate-pulse" /></div><h3 className="text-5xl font-[950] tracking-tighter uppercase leading-none">{title}</h3></div><div className="text-white">{children}</div></Card>
     </motion.div>
  </div>
);
...
const EditForm = ({ log, configs, onSave }) => {
  ...
  return (
    <div className="space-y-14 text-white">
       ...
       <Button size="lg" className="w-full h-24 rounded-[42px] shadow-2xl text-base font-black transition-all hover:scale-[1.02] shadow-black/50" onClick={() => onSave(log.logDate, c)}><Save size={32} className="mr-6" /> Register Override</Button>
    </div>
  );
};
...
const InsightCard = ({ Icon, label, val, suffix, color, index }) => (
  <StaggeredItem index={index}>
    <Card className="flex flex-col items-center text-center py-16 bg-bg-panel/50 border-white/5 shadow-[0_50px_100px_rgba(0,0,0,0.6)] group relative overflow-hidden transition-all duration-1000 hover:border-accent/40 hover:bg-white/[0.01]">
      <div className={cn("p-8 rounded-[48px] bg-white/[0.04] mb-12 transition-all duration-1000 group-hover:scale-110 group-hover:bg-accent/10 border border-white/5 shadow-2xl relative z-10", color)}><Icon size={48} /></div>
      ...
      <div className="text-[12px] font-black text-text-muted uppercase tracking-[0.6em] relative z-10 opacity-70">{suffix}</div>
